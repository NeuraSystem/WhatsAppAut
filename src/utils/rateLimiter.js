...
class RateLimiter {
    ...
    startRefillTimer() {
        ...
        logger.info(`Rate limiter iniciado: ${this.maxTokens} tokens, ${this.refillRate} refresco/min`);
    }
    ...
    resetClient(clientId) {
        ...
        for (const [apiType, limiter] of Object.entries(this.limiters)) {
            try {
                ...
                logger.error(`Error reseteando rate limit ${apiType} para ${clientId}:`, error);
                success = false;
            }
        }
...
async function withRateLimit(limiter, clientId, fn, tokensNeeded = 1) {
    ...
    if (!result.allowed) {
        const error = new Error(`Rate limit excedido. Intenta en ${Math.ceil(result.waitTime / 1000)} segundos.`);
