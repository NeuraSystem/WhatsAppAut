...
class ProactiveMonitor {
    ...
    async performHealthCheck() {
        ...
        try {
            ...
            logger.info(`Verificación de salud completada en ${endTime - startTime}ms`, {
            ...
            this.addAlert('critical', 'health_check_failed', 'Error en verificación de salud del sistema');
        }
    }
    ...
    async checkDatabaseHealth() {
        try {
            ...
            if (!this.healthChecks.database) {
                this.addAlert('critical', 'database_connection', 'No se puede conectar a la base de datos');
            }
            
        } catch (error) {
            this.healthChecks.database = false;
            this.addAlert('critical', 'database_error', `Error de base de datos: ${error.message}`);
        }
    }
    ...
    async checkAnthropicAPIHealth() {
        try {
            ...
            if (!this.healthChecks.anthropicAPI) {
                this.addAlert('critical', 'anthropic_api', 'API de Anthropic no disponible (Circuit Breaker abierto)');
            }
            
        } catch (error) {
            this.healthChecks.anthropicAPI = false;
            this.addAlert('warning', 'anthropic_api_check', `Error verificando API Anthropic: ${error.message}`);
        }
    }
        ...
        try {
            ...
            if (status) {
                ...
                if (status.state === 'open') {
                    this.addAlert('critical', 'circuit_breaker_open', 'Circuit Breaker abierto - API no disponible');
                } else if (status.state === 'half-open') {
                    this.addAlert('warning', 'circuit_breaker_half_open', 'Circuit Breaker en modo de prueba');
                }
            } else {
                this.healthChecks.circuitBreaker = false;
                this.addAlert('warning', 'circuit_breaker_missing', 'Circuit Breaker no inicializado');
            }
            
        } catch (error) {
            this.healthChecks.circuitBreaker = false;
            this.addAlert('warning', 'circuit_breaker_error', `Error en Circuit Breaker: ${error.message}`);
        }
    }
    ...
    async checkRateLimiterHealth() {
        try {
            ...
            for (const [apiType, apiStats] of Object.entries(stats)) {
                ...
                if (hitRate < 50) { // Si menos del 50% de requests son exitosos
                    this.addAlert('warning', 'rate_limit_high', `Alto rate limiting en ${apiType}: ${hitRate}% éxito`);
                }
            }
            
        } catch (error) {
            this.healthChecks.rateLimiter = false;
            this.addAlert('warning', 'rate_limiter_error', `Error en Rate Limiter: ${error.message}`);
        }
    }
    ...
    async calculateSystemMetrics() {
        try {
            ...
            if (failureStats.total_clients > 0) {
                this.metrics.errorRate = failureStats.clients_with_failures / failureStats.total_clients;
            }
            ...
            if (satisfactionResult.rows[0]?.avg_satisfaction) {
                this.metrics.userSatisfaction = parseFloat(satisfactionResult.rows[0].avg_satisfaction);
            }
    ...
    evaluateSystemHealth() {
        ...
        const healthyComponents = Object.values(this.healthChecks).filter(Boolean).length;
        ...
        const satisfactionPenalty = Math.max(0, (this.thresholds.minUserSatisfaction - this.metrics.userSatisfaction) * 10);
        
        healthScore = Math.max(0, componentHealth - errorPenalty - satisfactionPenalty);
    ...
    generateAlerts() {
        ...
        if (this.metrics.systemHealth < this.thresholds.minSystemHealth) {
            this.addAlert('critical', 'system_health_low', `Salud del sistema crítica: ${this.metrics.systemHealth}%`);
        }
        
        // Alertas por tasa de error
        if (this.metrics.errorRate >= this.thresholds.criticalErrorRate) {
            this.addAlert('critical', 'error_rate_high', `Tasa de error crítica: ${(this.metrics.errorRate * 100).toFixed(1)}%`);
        } else if (this.metrics.errorRate >= this.thresholds.warningErrorRate) {
            this.addAlert('warning', 'error_rate_elevated', `Tasa de error elevada: ${(this.metrics.errorRate * 100).toFixed(1)}%`);
        }
        
        // Alertas por satisfacción del usuario
        if (this.metrics.userSatisfaction < this.thresholds.minUserSatisfaction) {
            this.addAlert('warning', 'user_satisfaction_low', `Satisfacción del usuario baja: ${this.metrics.userSatisfaction}/10`);
        }
    }
    ...
    checkCriticalAlerts() {
        ...
        if (criticalAlerts.length > 0) {
            logger.error(`${criticalAlerts.length} alertas críticas sin atender:`, 
