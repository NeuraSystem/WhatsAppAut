...
class PerformanceMonitor {
    ...
    recordSourceStats(source, latency) {
        if (!this.metrics.sourceStats[source]) {
            this.metrics.sourceStats[source] = { count: 0, totalLatency: 0, errors: 0 };
        }
    ...
    checkAlerts(requestData) {
        ...
        if (source === 'claude_remote' && requestData.reason === 'local_unavailable') {
    ...
    getSummary() {
        ...
        const avgLatency = latencyValues.length > 0 ? 
            Math.round(totalLatency / latencyValues.length) : 0;
    ...
    getLatencyTrends() {
        ...
        Object.keys(this.metrics.hourlyStats).forEach(hour => {
            const stats = this.metrics.hourlyStats[hour];
            hourlyAverages[hour] = stats.count > 0 ? 
                Math.round(stats.totalLatency / stats.count) : 0;
        });
