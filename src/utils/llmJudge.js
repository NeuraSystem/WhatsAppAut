...
class LLMJudge {
    constructor() {
        ...
        this.evaluationCriteria = {
            relevance: {
                weight: 0.3,
                description: "¿Qué tan relevante es la respuesta a la pregunta del usuario?"
            },
            accuracy: {
                weight: 0.25,
     ...
     */
    async evaluateResponse(userQuery, botResponse, intent = 'unknown', context = {}) {
        try {
            // Verificar rate limiting
            const rateLimitCheck = apiRateLimiter.checkLimit('anthropic', 'llm_judge');
            ...
            const evaluationPrompt = this.buildEvaluationPrompt(userQuery, botResponse, intent, context);
...
`;
    ...
    validateEvaluation(evaluation) {
        const requiredFields = ['relevance', 'accuracy', 'clarity', 'completeness', 'tone'];
    ...
    updateMetrics(evaluation) {
        ...
        const currentTotal = this.metrics.averageScore * (this.metrics.totalEvaluations - 1);
        this.metrics.averageScore = (currentTotal + evaluation.overallScore) / this.metrics.totalEvaluations;
        
        // Actualizar distribución por puntuación
        const scoreRange = Math.floor(evaluation.overallScore);
        this.metrics.distributionByScore[scoreRange] = (this.metrics.distributionByScore[scoreRange] || 0) + 1;
    ...
    getResponsesNeedingImprovement(threshold = 6) {
        ...
        return poorResponses.sort((a, b) => a.evaluation.overallScore - b.evaluation.overallScore);
    }
    ...
    cleanupCache() {
        ...
        if (cleaned > 0) {
            logger.info(`Cache de evaluaciones limpiado: ${cleaned} entradas eliminadas`);
        }
