...
class ContextCache {
    ...
    async initializeStaticCache(dbConnection) {
        ...
        try {
            ...
            this.staticCache.businessInfo = {
                nombre: 'Salva Cell',
                servicios: ['Diagnóstico', 'Cambio de pantallas', 'Reparación de baterías', 'Accesorios'],
            ...
            this.staticCache.catalogIndex = await this.buildCatalogIndex(dbConnection);
    ...
    async buildCatalogIndex(dbConnection) {
        try {
            ...
            const result = await dbConnection.query('SELECT * FROM reparaciones ORDER BY precio ASC');
            const rows = result.rows;
                ...
                rows.forEach(product => {
                    ...
                    if (!index.byRepairType[repairType]) index.byRepairType[repairType] = [];
    ...
    extractProductKeywords(product) {
        ...
        // Keywords del modelo
        const modelWords = product.modelo_celular.toLowerCase()
            .split(/[\s\-_]+/)
            .filter(word => word.length > 1);
        keywords.push(...modelWords);

        // Keywords del tipo de reparación
        const repairWords = product.tipo_reparacion.toLowerCase()
            .split(/[\s\-_]+/)
