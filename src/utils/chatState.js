...
function pauseChat(chatId, durationInMinutes) {
    ...
    const timer = setTimeout(() => {
        resumeChat(chatId);
        logger.info(`Pausa expirada. El bot se ha reanudado automáticamente para el chat ${chatId}.`);
    }, durationInMs);

    pausedChats.set(chatId, timer);
    logger.info(`Bot pausado para el chat ${chatId} durante ${durationInMinutes} minutos.`);
}
