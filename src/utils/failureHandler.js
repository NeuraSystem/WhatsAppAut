...
class FailureHandler {
    constructor() {
        ...
        this.escalationMessages = {
            ...
            escalationInstructions: "Para hablar con un experto humano, escribe 'CONTACTAR HUMANO' o llámanos directamente. Mientras tanto, seguiré aquí por si necesitas información básica."
        };
    }
    ...
    async recordFailure(phoneNumber, userQuery, errorType = 'unknown') {
        try {
            ...
            try {
                ...
                if (lastFailureTime && (now - lastFailureTime) > this.config.failureResetTime) {
                    consecutiveFailures = 0;
                    logger.info(`Reseteando contador de fallos para ${phoneNumber} - han pasado más de 1 hora`);
                }
                ...
                logger.warn(`Fallo registrado para ${phoneNumber}`, {
                    ...
                    shouldEscalate: consecutiveFailures >= this.config.maxConsecutiveFailures
                });

                const shouldEscalate = consecutiveFailures >= this.config.maxConsecutiveFailures;
                
                return {
                    consecutiveFailures,
                    shouldEscalate,
                    escalationMessage: shouldEscalate ? this.getEscalationMessage() : null,
        ...
        try {
            ...
            try {
                ...
                if (result.rowCount > 0) {
                    logger.info(`Contador de fallos reseteado para ${phoneNumber} - respuesta exitosa`);
                }
    ...
    async checkFailureStatus(phoneNumber) {
        try {
            ...
            try {
                ...
                const shouldReset = lastFailure && (now - lastFailure) > this.config.failureResetTime;
                const effectiveFailures = shouldReset ? 0 : failures;

                return {
                    consecutiveFailures: effectiveFailures,
                    shouldEscalate: effectiveFailures >= this.config.maxConsecutiveFailures,
