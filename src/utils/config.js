...
const config = {
    ...
    performance: {
        ...
        memoryThreshold: parseFloat(process.env.PERFORMANCE_MEMORY_THRESHOLD) || 0.75,
        responseTimeTarget: parseInt(process.env.PERFORMANCE_RESPONSE_TIME_TARGET, 10) || 1500,
    },

    // Configuración de Resilience
    resilience: {
        enableCircuitBreaker: process.env.RESILIENCE_ENABLE_CIRCUIT_BREAKER === 'true',
        enableGracefulDegradation: process.env.RESILIENCE_ENABLE_GRACEFUL_DEGRADATION === 'true',
        emergencyProtocols: process.env.RESILIENCE_EMERGENCY_PROTOCOLS === 'true',
        
        // Circuit Breaker specific configurations
        circuitBreaker: {
            failureThreshold: parseInt(process.env.RESILIENCE_CB_FAILURE_THRESHOLD, 10) || 5,
            successThreshold: parseInt(process.env.RESILIENCE_CB_SUCCESS_THRESHOLD, 10) || 3,
            timeout: parseInt(process.env.RESILIENCE_CB_TIMEOUT, 10) || 30000, // 30 seconds
            enableAdaptiveThreshold: process.env.RESILIENCE_CB_ADAPTIVE_THRESHOLD === 'true',
            adaptiveWindow: parseInt(process.env.RESILIENCE_CB_ADAPTIVE_WINDOW, 10) || 300000, // 5 minutes
            responseTimeThreshold: parseInt(process.env.RESILIENCE_CB_RESPONSE_TIME_THRESHOLD, 10) || 5000, // 5 seconds
            enableResponseTimeCircuit: process.env.RESILIENCE_CB_ENABLE_RESPONSE_TIME_CIRCUIT === 'true',
            recoveryTimeout: parseInt(process.env.RESILIENCE_CB_RECOVERY_TIMEOUT, 10) || 10000, // 10 seconds
            halfOpenMaxAttempts: parseInt(process.env.RESILIENCE_CB_HALF_OPEN_MAX_ATTEMPTS, 10) || 3,
        },

        // Graceful Degradation specific configurations
        gracefulDegradation: {
            criticalServices: process.env.RESILIENCE_GD_CRITICAL_SERVICES ? process.env.RESILIENCE_GD_CRITICAL_SERVICES.split(',') : ['whatsappClient', 'agentExecutor', 'database'],
            importantServices: process.env.RESILIENCE_GD_IMPORTANT_SERVICES ? process.env.RESILIENCE_GD_IMPORTANT_SERVICES.split(',') : ['embeddings', 'cache', 'logging'],
            optionalServices: process.env.RESILIENCE_GD_OPTIONAL_SERVICES ? process.env.RESILIENCE_GD_OPTIONAL_SERVICES.split(',') : ['analytics', 'metrics'],
            
            degradationThresholds: {
                memory: parseFloat(process.env.RESILIENCE_GD_MEMORY_THRESHOLD) || 0.85,
                cpu: parseFloat(process.env.RESILIENCE_GD_CPU_THRESHOLD) || 0.90,
                responseTime: parseInt(process.env.RESILIENCE_GD_RESPONSE_TIME_THRESHOLD, 10) || 5000,
                errorRate: parseFloat(process.env.RESILIENCE_GD_ERROR_RATE_THRESHOLD) || 0.15
            },
            recoveryCheckInterval: parseInt(process.env.RESILIENCE_GD_RECOVERY_CHECK_INTERVAL, 10) || 30000,
            stabilityRequirement: parseInt(process.env.RESILIENCE_GD_STABILITY_REQUIREMENT, 10) || 5,
        }
    },
    ...
    intelligence: {
        ...
        uncertaintyThreshold: parseFloat(process.env.INTELLIGENCE_UNCERTAINTY_THRESHOLD) || 0.7,
        learningThreshold: parseInt(process.env.INTELLIGENCE_LEARNING_THRESHOLD, 10) || 3,
        adminTimeout: parseInt(process.env.INTELLIGENCE_ADMIN_TIMEOUT, 10) || 300000, // 5 minutes
    }
};
