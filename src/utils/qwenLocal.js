...
class QwenLocalClient {
    constructor() {
      ...
      parseInt(process.env.LOCAL_LLM_HEALTH_CHECK_INTERVAL) || 30000;
    }
    ...
    async checkHealth() {
      ...
      this.isAvailable
        ...
        try {
            ...
            const hasQwen = response.data.models?.some(
                (model) =>
            ...
            if (!hasQwen) {
                logger.warn(
                    'Qwen2.5-1.5B no encontrado en Ollama. Modelos disponibles:',
        ...
        try {
            ...
            return {
                ...
                tokensPerSecond: response.data.eval_count
                    ? Math.round((response.data.eval_count / latency) * 1000)
                    : 0
            };
        } catch (error) {
            const endTime = Date.now();
