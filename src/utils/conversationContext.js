...
const { normalizeQuery, normalizeDeviceName, normalizeRepairType, normalizeQualityType } = require('./fuzzyMatcher');
...
class ConversationContext {
    constructor() {
        ...
        this.config = {
            ...
            contextResetWords: [           // Palabras que resetean el contexto
                'hola', 'buenas', 'saludos', 'nueva consulta', 'otra pregunta',
                'cambio de tema', 'ahora sobre', 'quiero preguntar'
            ],
            anaphoricanWords: [            // Palabras que indican referencia anafórica
                'ese', 'esa', 'eso', 'el', 'la', 'lo', 'genérica', 'original',
                'más barata', 'más cara', 'otra', 'también', 'y'
            ]
        };
        ...
        this.entityTypes = {
            phoneModels: [
                'iphone', 'samsung', 'galaxy', 'xiaomi', 'redmi', 'huawei', 
                'honor', 'motorola', 'moto', 'lg', 'nokia', 'oppo', 'vivo'
            ],
            repairTypes: [
                'pantalla', 'screen', 'display', 'bateria', 'battery', 'camara',
                'bocina', 'speaker', 'microfono', 'cargador', 'puerto', 'tactil',
                'vidrio', 'lcd', 'oled'
            ],
            qualityTypes: [
                'original', 'genérica', 'generico', 'oem', 'aftermarket', 
                'compatible', 'económica', 'premium', 'barata', 'cara'
            ]
        };
    }
    ...
    updateContext(clientId, userMessage, intent, botResponse) {
        ...
        if (context.messages.length > this.config.maxMessagesInContext) {
            context.messages = context.messages.slice(-this.config.maxMessagesInContext);
        }
    ...
    resolveAnaphora(clientId, userMessage) {
        ...
        for (const [anaphora, resolution] of Object.entries(resolutions)) {
            if (userMessage.toLowerCase().includes(anaphora)) {
                ...
                logger.info(`Anáfora resuelta para ${clientId}: "${anaphora}" -> "${resolution}"`);
                break;
            }
        }
    ...
    extractEntities(message) {
        ...
        const priceMatch = message.match(/\$?(\d{1,4}(?:,\d{3})*(?:\.\d{2})?)\s*pesos?/i);
        if (priceMatch) {
    ...
    cleanupExpiredContexts() {
        ...
        if (cleaned > 0) {
            logger.info(`Limpieza de contextos: ${cleaned} contextos expirados eliminados`);
        }
