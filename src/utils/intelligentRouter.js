...
class IntelligentRouter {
    ...
    async processQuery(userQuery, context) {
        ...
        try {
            ...
            logger.info(`Routing decision for "${userQuery.substring(0, 50)}...": ${routingDecision.useLocal ? 'LOCAL' : 'REMOTE'}`, {
            ...
            if (routingDecision.useLocal) {
                ...
                result = await this.processWithLocal(userQuery, context, routingDecision);
                ...
                if (result.shouldFallbackToRemote) {
                    ...
                    result = await this.processWithRemote(userQuery, context, 'local_fallback');
                    this.metrics.failovers++;
                }
            } else {
                // Usar Claude remoto directamente
                result = await this.processWithRemote(userQuery, context, routingDecision.reason);
            }
            ...
            try {
                const fallbackResult = await this.processWithRemote(userQuery, context, 'emergency_fallback');
                this.metrics.failovers++;
                return {
                    ...fallbackResult,
                ...
                return {
                    respuesta_completa: "¡Ups! Algo no salió como esperaba. Ya estoy trabajando para solucionarlo. Por favor, intenta de nuevo en un momento. 😊",
    ...
    analyzeComplexity(userQuery, context) {
        ...
        const complexWords = [
            'comparar', 'diferencia', 'recomendacion', 'opinion', 'mejor', 'peor',
            'problema', 'error', 'falla', 'no funciona', 'se descompuso',
            'calidad', 'garantia', 'durabilidad', 'original', 'generico'
        ];
        ...
        const emotionalWords = [
            'urgente', 'rapido', 'ayuda', 'problema', 'molesto', 'frustrado',
            'enojado', 'preocupado', 'favor', 'por favor'
        ];
    ...
    analyzeIntention(userQuery) {
        ...
        const localIndicators = [
            'precio', 'cuanto', 'costo', 'vale',
            'ubicacion', 'donde', 'direccion',
            'hola', 'buenas', 'saludo',
            'gracias', 'thank'
        ];
        ...
        const remoteIndicators = [
            'recomendacion', 'recomienda', 'mejor', 'opinion',
            'problema', 'falla', 'error', 'no funciona',
            'comparar', 'diferencia', 'vs',
            'queja', 'molesto', 'mal servicio'
        ];
    ...
    async processWithLocal(userQuery, context, routingInfo) {
        ...
        try {
            ...
            const extractedInfo = await this.qwenClient.extractQueryInfo(userQuery, optimizedContext);
            
            // Buscar productos si es necesario
            let productInfo = null;
            if (extractedInfo.necesita_busqueda_bd || 
            ...
            const qualityCheck = this.evaluateResponseQuality(response, extractedInfo, latency);
            ...
            return {
                ...
                debe_presentarse_ia: optimizedContext.esNuevo && !optimizedContext.yaSePresentoIA,
    ...
    findProductInRelevant(relevantProducts, brand, repairType) {
        ...
        let found = relevantProducts.find(product => 
    ...
    evaluateResponseQuality(response, extractedInfo, latency) {
        ...
        if (response.respuesta_completa?.endsWith('...') || 
    ...
    getMetrics() {
        const localPercentage = this.metrics.totalRequests > 0 ? 
            (this.metrics.localRequests / this.metrics.totalRequests) * 100 : 0;
        
        const failoverRate = this.metrics.totalRequests > 0 ?
            (this.metrics.failovers / this.metrics.totalRequests) * 100 : 0;
