...
class AbusePatternDetector {
    ...
    analyzePattern(clientId, pattern) {
        ...
        if (suspicionScore >= 70) {
            this.flagAsSuspicious(clientId, 'high_suspicion_score', { score: suspicionScore });
        } else if (suspicionScore >= 50) {
            this.flagAsSuspicious(clientId, 'medium_suspicion_score', { score: suspicionScore });
        }
    }
    ...
    removeFromBlacklist(clientId) {
        ...
        if (removed) {
            this.metrics.usersBlacklisted = Math.max(0, this.metrics.usersBlacklisted - 1);
...
class AdvancedRateLimiter {
    ...
    getUserLimits(userRole = 'guest') {
        ...
        return {
            requestsPerMinute: this.config[`${roleKey}RequestsPerMinute`] || this.config.defaultRequestsPerMinute,
            burstLimit: this.config[`${roleKey}BurstLimit`] || this.config.defaultBurstLimit
        };
    }
    ...
    async checkRateLimit(clientId, userRole = 'guest', requestInfo = {}) {
        ...
        try {
            ...
            if (this.config.enableAbuseDetection && this.abuseDetector.isBlacklisted(clientId)) {
            ...
            const { bucket, window, metrics } = this.getOrCreateClientBucket(clientId, userRole);
    ...
    updateClientLimits(clientId, newRole) {
        const { bucket, window, metrics } = this.getOrCreateClientBucket(clientId, newRole);
    ...
    getClientStats(clientId) {
        ...
        return {
            ...
            requestsPerMinute: metrics.totalRequests / ((Date.now() - metrics.firstRequest) / 60000)
        };
    }
    ...
    getSystemMetrics() {
        ...
        return {
            ...
            uptime,
            requestsPerMinute,
            blockRatio: this.metrics.blockedRequests / Math.max(1, this.metrics.totalRequests),
    ...
    middleware() {
        return async (req, res, next) => {
            ...
            try {
                ...
                const clientId = req.headers[this.config.clientIdHeader] || 
                               req.ip || 
                ...
                // Check rate limits
                const result = await this.rateLimiter.checkRateLimit(clientId, userRole, requestInfo);
                ...
                if (result.clientInfo) {
                    res.set({
                        ...
                        'X-RateLimit-Reset': (Date.now() + (result.clientInfo.window.resetIn || 0)).toString()
                    });
                }
