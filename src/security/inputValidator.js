...
class BusinessRuleValidator extends ValidationRule {
    ...
    async validate(input, context = {}) {
        ...
        if (context.messageCount && context.timeWindow) {
            const messagesPerMinute = context.messageCount / (context.timeWindow / 60000);
            if (messagesPerMinute > 10) {
                return this.createResult(
                    false,
                    'Message rate too high',
                    input,
                    { level: 'medium', type: 'business_violation', subtype: 'rate_abuse' }
                );
            }
        }
...
class InputValidationChain {
    ...
    addValidator(validator) {
        ...
        logger.info(`✅ Validator added: ${validator.name} (${validator.severity})`);
    }
    ...
    getMetrics() {
        ...
        return {
            ...
            sanitizationRate: this.metrics.sanitizations / this.metrics.totalValidations,
            criticalBlockRate: this.metrics.criticalBlocks / this.metrics.totalValidations,
            validatorCount: this.validators.size
        };
    }
}
...
class ValidationMiddleware {
    ...
    async middleware(req, res, next) {
        ...
        try {
            ...
            const validationResult = await this.validationChain.validateInput(input, context);
...
class InputValidatorFactory {
    ...
    static createMiddleware(type = 'default') {
        const chain = this[`create${type.charAt(0).toUpperCase() + type.slice(1)}`]();
