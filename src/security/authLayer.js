...
class WhatsAppUser {
    ...
    isBlocked() {
        return (
            this.metadata.blockedUntil && Date.now() < this.metadata.blockedUntil
        );
    }
...
class Session {
    ...
    generateToken(type = 'access', expiresIn = 3600000) {
    // 1 hour default
...
class AuthenticationManager {
    ...
    // GUEST ROLE - Default for new users
        const guestRole = new Role('guest', 'Default guest user', 0)
            .addPermission(
                new Permission('basic_chat', 'Basic chat interactions', 'chat', 'send')
            )
            .addPermission(
                new Permission('price_inquiry', 'Price inquiries', 'prices', 'read')
            )
            .addPermission(
                new Permission(
                    'service_info',
        ...
        const userRole = new Role('user', 'Verified user', 10)
            .addPermission(
                new Permission('basic_chat', 'Basic chat interactions', 'chat', '*')
            )
            .addPermission(
                new Permission('price_inquiry', 'Price inquiries', 'prices', 'read')
            )
            .addPermission(
                new Permission('service_info', 'Service information', 'services', '*')
            )
            .addPermission(
                new Permission('order_creation', 'Create orders', 'orders', 'create')
            )
            .addPermission(
                new Permission(
                    'warranty_check',
        ...
        const vipRole = new Role('vip', 'VIP customer', 20)
            .addPermission(
                new Permission('all_basic', 'All basic permissions', '*', 'read')
            )
            .addPermission(
                new Permission('priority_support', 'Priority support', 'support', '*')
            )
            .addPermission(
                new Permission(
                    'special_pricing',
                    'Special pricing access',
                    'prices',
                    '*'
                )
            )
            .addPermission(
                new Permission('order_management', 'Order management', 'orders', '*')
            );

        // ADMIN ROLE - System administrators
        const adminRole = new Role('admin', 'System administrator', 100)
            .addPermission(
                new Permission('full_access', 'Full system access', '*', '*')
            )
            .addPermission(
                new Permission('user_management', 'User management', 'users', '*')
            )
            .addPermission(
                new Permission('system_config', 'System configuration', 'system', '*')
            )
            .addPermission(
                new Permission('audit_access', 'Audit log access', 'audit', '*')
            );
    ...
    async authenticateUser(
        ...
        try {
            ...
            // Generate tokens
            const accessToken = session.generateToken(
                'access',
                this.config.tokenLifetime
            );
            const refreshToken = session.generateToken(
                'refresh',
                this.config.tokenLifetime * 24
            ); // 24 hours
    ...
    getMetrics() {
        ...
        return {
            ...
            uptime,
            authSuccessRate:
        1 - this.metrics.authFailures / Math.max(1, this.metrics.authAttempts),
...
class AuthMiddleware {
    ...
    async authenticate(phoneNumber, name, pushName, metadata = {}) {
        ...
        return await this.authManager.authenticateUser(
            phoneNumber,
            name,
            pushName,
            metadata
    ...
    async authorize(sessionId, resource, action, context = {}) {
        ...
        return await this.authManager.authorizeAction(
            sessionId,
            resource,
            action,
            context
