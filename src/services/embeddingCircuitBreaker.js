...
class EmbeddingCircuitBreaker {
  ...
  async execute(operation, context = {}) {
    ...
    try {
      ...
      if (!canExecute.allowed) {
        return await this.handleBlockedOperation(
          canExecute.reason,
          context,
          operationId,
        );
      ...
      if (this.hasFallbackStrategy(context)) {
        logger.info(
          `🔄 Ejecutando estrategia de fallback para: ${context.type || "unknown"}`,
        );
  ...
  canExecuteInDegradedMode(context) {
    ...
    if (
      this.metrics.degradedModeStartTime &&
      currentTime - this.metrics.degradedModeStartTime >
        this.config.salvaCellContext.maxDegradationTime
    ) {
  ...
  recordFailure(operationId, duration, error, context) {
    ...
    logger.warn(
      `❌ Operación fallida: ${operationId} (${duration}ms) - ${error.message}`,
    );
  }
  ...
  transitionToOpen() {
    ...
    this.notifyStateChange(
      "OPEN",
      "Service is unavailable - circuit breaker opened",
    );
  }
  ...
  shouldAttemptRecovery(currentTime) {
    return (
      currentTime - this.metrics.stateChangeTime >= this.config.recoveryTime
    );
  }
  ...
  isOperationCritical(context) {
    ...
    return this.config.salvaCellContext.criticalOperations.includes(
      operationType,
    );
  }
  ...
  initializeFallbackStrategies() {
    ...
    this.fallbackStrategies.set("price_query", async (context, error) => {
      ...
      return {
        ...
        message:
          "Información básica de precios disponible. Para detalles específicos, contacta directamente.",
        fallback: true,
    ...
    this.fallbackStrategies.set(
      ...
      async (context, error) => {
        logger.info(
          "📱 Ejecutando fallback para clasificación de dispositivos",
        );

        return {
          type: "fallback_response",
          message:
            "Clasificación básica disponible. Especifica marca y modelo para mejor asistencia.",
          fallback: true,
          originalError: error.message,
        };
      },
    ...
    this.fallbackStrategies.set("conversation", async (context, error) => {
      ...
      return {
        type: "fallback_response",
        message:
          "Servicio temporalmente limitado. Tu consulta ha sido registrada.",
        fallback: true,
        originalError: error.message,
      };
    });
  ...
  hasFallbackStrategy(context) {
    const operationType =
      context.type || context.operationType || "conversation";
  ...
  recordOperationAttempt(operationId, context) {
    logger.debug(`🔄 Iniciando operación: ${operationId}`, {
      context,
      state: this.currentState,
    });
  }
  ...
  notifyStateChange(newState, message) {
    ...
    logger.warn(
      `🚨 Circuit Breaker State Change: ${notification.previousState} → ${newState}`,
      notification,
    );
  ...
  getStatus() {
    ...
    const recentSuccesses = recentOperations.filter(
      (op) => op.type === "success",
    ).length;
    const recentFailures = recentOperations.filter(
      (op) => op.type === "failure",
    ).length;
    const recentFallbacks = recentOperations.filter(
      (op) => op.type === "fallback",
    ).length;
      ...
      metrics: {
        ...
        recentSuccessRate:
          recentOperations.length > 0
            ? recentSuccesses / recentOperations.length
            : 0,
  ...
  calculateHealthScore() {
    ...
    const recentOps = this.operationHistory.filter(
      (op) => op.timestamp >= recentWindow,
    );

    if (recentOps.length === 0) return 1.0;

    const successRate =
      recentOps.filter((op) => op.type === "success").length / recentOps.length;
    const stateScore = {
