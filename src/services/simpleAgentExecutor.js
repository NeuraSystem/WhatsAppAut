...
class SimpleAgentExecutor {
    constructor(options = {}) {
        this.primaryLlm = new ChatOllama({
            ...
            temperature: options.temperature || config.orchestrator.temperature || 0.1,
        });

        this.fallbackLlm = new ChatOllama({
            baseUrl: config.orchestrator.ollamaBaseUrl,
            model: config.orchestrator.ollamaFallbackModel || 'qwen2.5:1.5b-instruct-q4_0',
            temperature: options.temperature || config.orchestrator.temperature || 0.1,
        });

        this.tools = tools;
        this.guardrails = new Guardrails();
        this.maxIterations = options.maxToolCalls || config.orchestrator.maxToolCalls || 3;
        ...
        logger.info('🧠 SimpleAgentExecutor inicializado con fallback y rate limiting.');
    }
    ...
    async execute(input) {
        try {
            ...
            if (!input || typeof input !== 'string' || input.trim().length === 0) {
                ...
                return {
                    response: 'Lo siento, no puedo procesar esa consulta. ¿Podrías reformularla?',
                    success: false,
                    cached: false
                };
            }
            ...
            try {
                ...
                logger.warn('Error en el LLM primario. Intentando con el fallback...', error.message);
                try {
                    response = await this.executeWithLLM(this.fallbackLlm, input);
                    logger.info('✅ Respuesta exitosa del LLM de fallback');
                } catch (fallbackError) {
                    logger.error('Error en el LLM de fallback. No se pudo procesar la solicitud.', fallbackError.message);
                    throw new OllamaError('Los modelos de lenguaje no están disponibles.');
                }
            }
            ...
            logger.error('SalvaCellAgentExecutor: Error ejecutando el agente:', error);
            throw error;
        }
    }
