...
class MetadataEnhancer {
    constructor(options = {}) {
        ...
        this.salvaCellSchema = {
            ...
            main_intent: { 
                type: 'string', 
                required: true,
                enum: ['price_inquiry', 'location_request', 'availability_check', 'warranty_claim', 'general_inquiry']
            },
            ...
            device_brand: { 
                type: 'string', 
                required: false,
                enum: ['iphone', 'samsung', 'motorola', 'xiaomi', 'huawei', 'lg', 'other']
            },
    ...
    async enhanceMetadata(rawMetadata, context = {}) {
        ...
        try {
            ...
            if (this.config.autoEnrichment) {
                enhancedMetadata = this.applyAutoEnrichment(
                    enhancedMetadata, 
                    context
                );
    ...
    validateField(fieldName, value, schema) {
        ...
        if (!this.isValidType(value, schema.type)) {
            ...
            if (converted.success) {
                ...
                result.isValid = false;
                result.errors.push(`Invalid type for ${fieldName}: expected ${schema.type}`);
                return result;
            }
        }
    ...
    applyAutoEnrichment(metadata, context) {
        ...
        if (!enriched.confidence_score) {
            enriched.confidence_score = this.calculateConfidenceScore(metadata, context);
        }
        ...
        if (!enriched.device_brand && enriched.device_mentioned) {
            enriched.device_brand = this.extractDeviceBrand(enriched.device_mentioned);
        }
    ...
    normalizeIntent(intent) {
        ...
        const mapped = this.mapToValidEnum(intent, [
            'price_inquiry', 'location_request', 'availability_check', 
            'warranty_claim', 'general_inquiry'
        ]);
    ...
    mapServiceType(serviceMentioned) {
        ...
        if (lower.includes('pantalla') || lower.includes('display')) return 'pantalla';
        if (lower.includes('batería') || lower.includes('battery')) return 'batería';
    ...
    createFallbackMetadata(rawMetadata, context) {
        return {
            client_id: this.normalizeClientId(rawMetadata.client_id || rawMetadata.clientId || 'unknown'),
            timestamp: Date.now(),
     ...
     */
    getEnhancementStats() {
        const totalProcessed = this.metrics.successfulValidations + this.metrics.failedValidations;
        
        return {
            totalValidations: this.metrics.totalValidations,
            successRate: totalProcessed > 0 ? (this.metrics.successfulValidations / totalProcessed) : 0,
