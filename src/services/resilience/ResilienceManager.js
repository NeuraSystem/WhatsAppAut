...
class ResilienceManager {
    constructor(config = {}) {
        ...
        logger.info('🛡️ ResilienceManager initialized with comprehensive fault tolerance');
    }
    ...
    initializeServices() {
        ...
        this.circuitBreakers.set('anthropic', new CircuitBreaker(
            this.executeAnthropicRequest.bind(this),
            {
        ...
        this.circuitBreakers.set('embedding', new CircuitBreaker(
            this.executeEmbeddingRequest.bind(this),
            {
        ...
        this.circuitBreakers.set('database', new CircuitBreaker(
            this.executeDatabaseRequest.bind(this),
            {
        ...
        logger.info('🔧 Services initialized with circuit breakers and rate limiters');
    }
    ...
    async executeWithProtection(serviceName, clientId, operation, options = {}) {
        ...
        try {
            ...
            logger.debug(`✅ Protected execution ${executionId} completed successfully`);
            return result;
            
        } catch (error) {
            timer.end();
            await this.handleExecutionError(serviceName, clientId, error, executionId);
            throw error;
        }
    }
    ...
    async checkServiceHealth(serviceName) {
        ...
        if (serviceHealth && serviceHealth.status === 'unhealthy') {
            ...
            if (timeSinceFailure < recoveryTime) {
                throw new Error(`Service ${serviceName} is currently unhealthy. Retry in ${Math.ceil((recoveryTime - timeSinceFailure) / 1000)}s`);
            }
        }
    }
    ...
    async executeWithRetry(operation, options = {}) {
        ...
        for (let attempt = 1; attempt <= retryConfig.maxAttempts; attempt++) {
            try {
                ...
                logger.warn(`🔄 Retry attempt ${attempt}/${retryConfig.maxAttempts} after ${delay}ms for error: ${error.message}`);
    ...
    async executeAnthropicRequest(operation) {
        return await this.executeWithTimeout(operation, this.config.timeouts.anthropic);
    }

    async executeEmbeddingRequest(operation) {
        return await this.executeWithTimeout(operation, this.config.timeouts.embedding);
    }

    async executeDatabaseRequest(operation) {
        return await this.executeWithTimeout(operation, this.config.timeouts.database);
    }
    ...
    updateServiceHealth(serviceName, status, error = null) {
        this.systemHealth.services[serviceName] = {
            status,
            lastCheck: Date.now(),
            lastFailure: status === 'unhealthy' ? Date.now() : 
        ...
        for (const [name, limiter] of this.rateLimiters) {
            if (name === 'client') {
                ...
                status[name] = {
                    ...
                    maxRequests: limiter.maxRequests,
                    windowMs: limiter.windowMs,
                    utilization: Math.round((limiter.currentCount / limiter.maxRequests) * 100)
                };
            }
        }
        return status;
