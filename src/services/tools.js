  ...
  for (const field of schema.required) {
    if (!(field in args) || args[field] === null || args[field] === undefined) {
      ...
      validation.errors.push(
        `Campo requerido faltante para ${toolName}: ${field}`,
      );
    }
  }
  ...
  for (const [field, value] of Object.entries(args)) {
    if (schema.types && field in schema.types) {
      ...
      if (typeof value !== expectedType) {
        validation.errors.push(
          `Tipo incorrecto para ${toolName}.${field}: esperado ${expectedType}, recibido ${typeof value}`,
        );
        validation.isValid = false;
      }
    }
...
class EnhancedLangChainEmbeddingAdapter {
  constructor(enhancedEmbeddingEngine, defaultTaskType = "document") {
    ...
    logger.info(
      `Enhanced Tools Adapter inicializado - Tipo de tarea por defecto: ${defaultTaskType}`,
    );
  }
  ...
  async generate(texts) {
    try {
      ...
      logger.error(
        "Error en EnhancedLangChainEmbeddingAdapter (Tools):",
        error,
      );
      throw error;
    }
...
class CrossSourceValidator {
  ...
  async validatePriceConsistency(semanticResult, originalQuery) {
    try {
      ...
      const postgresResult = await this.postgresConnector.query(postgresQuery, [
        `%${deviceInfo}%`,
      ]);
      ...
      const postgresPrices = postgresResult.rows.map((row) =>
        parseFloat(row.precio),
      );

      if (semanticPrices.length > 0 && postgresPrices.length > 0) {
        const consistencyData = this.comparePrices(
          semanticPrices,
          postgresPrices,
        );
        const result = {
          isConsistent: consistencyData.isConsistent,
          warning: consistencyData.isConsistent
            ? null
            : "Posible inconsistencia de precios detectada entre fuentes",
  ...
  comparePrices(semanticPrices, postgresPrices) {
    ...
    const confidence = Math.max(
      0,
      Math.min(1, overlapRatio - priceVariance * 0.1),
    );
  ...
  async validateProductAvailability(semanticResult, originalQuery) {
    try {
      const deviceInfo = this.extractDeviceInfo(semanticResult.content);
      if (!deviceInfo)
        return { isAvailable: true, source: "no_device_detected" };
      ...
      const result = await this.postgresConnector.query(availabilityQuery, [
        `%${deviceInfo}%`,
      ]);

      const availabilityData = result.rows[0];
      const isAvailable =
        availabilityData.count > 0 && availabilityData.availability_ratio > 0.5;
...
const performanceMonitor = {
  startTimer: (operation, context = {}) => {
    ...
    return {
      ...
      start,
        ...
        logger.debug(
          `⏱️ Performance: ${operation} took ${duration.toFixed(2)}ms`,
          context,
        );
        return duration;
      },
  ...
  measureAsync: async (operation, asyncFunction, context = {}) => {
    ...
    try {
      ...
      logger.warn(
        `⚠️ Performance: ${operation} failed after ${duration.toFixed(2)}ms`,
...
const knowledgeSearchTool = new DynamicTool({
  ...
  description:
    "Útil para responder preguntas sobre precios de reparaciones, disponibilidad de productos, tiempos de servicio y otros conocimientos generales del negocio. La entrada debe ser la pregunta específica del cliente.",
  func: async (query) => {
    const schema = {
      required: ["query"],
      types: {
    ...
    return await performanceMonitor.measureAsync(
      "consultar_conocimiento_avanzado",
      async () => {
        ...
        logger.debug(
          `🎯 Límites optimizados para conocimiento: base=${limitOptimization.baseLimit}, max=${limitOptimization.maxLimit}, contexto=${contextType}`,
        );

        // 2. BÚSQUEDA DETERMINÍSTICA CON LÍMITES OPTIMIZADOS
        const stabilizedResults =
          await deterministicSearchEngine.performStabilizedSearch(
            collection,
            query,
        ...
        if (
          !stabilizedResults.documents ||
          !stabilizedResults.documents.length ||
          !stabilizedResults.documents[0].length
        ) {
        ...
        for (let i = 0; i < enrichedResults.documents[0].length; i++) {
          const result = {
            ...
            metadata: enrichedResults.metadatas
              ? enrichedResults.metadatas[0][i]
              : {},
          ...
          const validation =
            await crossSourceValidator.validatePriceConsistency(result, query);
          if (!validation.isConsistent && validation.warning) {
            ...
            logger.warn(
              `⚠️ Inconsistencia detectada: ${validation.warning} (device: ${validation.deviceInfo})`,
            );
          }
        ...
        if (enrichedResults.globalMarkdownInfo) {
          ...
          if (
            (contextType === "WARRANTY_LOOKUP" ||
              contextType === "MARKDOWN_CONTEXT") &&
            globalInfo.standardInfo
          ) {
        ...
        const validationWarnings = results.filter(
          (r) => r.metadata.consistencyWarning,
        ).length;
        ...
        logger.info(
          `✅ Contexto avanzado encontrado: ${enhancedContext.length} caracteres (límites: ${limitOptimization.baseLimit}, consenso: ${consensusQuality}, validaciones: ${validationWarnings})`,
        );
...
function determineKnowledgeContext(query) {
  ...
  if (
    lowerQuery.includes("garantía") ||
    lowerQuery.includes("original") ||
    lowerQuery.includes("genérica")
  ) {
  ...
  if (
    ...
    lowerQuery.includes("minutos") ||
    lowerQuery.includes("horas")
  ) {
...
async function knowledgeSearchFallback(query) {
  try {
    ...
    if (
      ...
      logger.info(
        `Fallback exitoso para conocimiento: ${context.length} caracteres`,
      );
      return context;
    } else {
      return "No se encontró información específica sobre esa consulta en la base de conocimientos.";
    ...
    if (
      fallbackError.message.includes("Collection") &&
...
const customerHistoryTool = new DynamicTool({
  ...
  description:
    "Obtiene el historial de interacciones y reparaciones de un cliente específico usando su número de teléfono (ID de cliente). La entrada debe ser el ID del cliente.",
...
const escalateToHumanTool = new DynamicTool({
  ...
  description:
    "Se debe usar cuando el cliente está muy molesto, presenta una queja grave, o cuando el bot ha fallado repetidamente en entender la consulta. La entrada debe ser un resumen del caso.",
...
const preciseSearchTool = new DynamicTool({
  ...
  description:
    "Herramienta ESPECIALIZADA para consultas de precios. Usa múltiples estrategias (búsqueda exacta, difusa) para encontrar precios precisos. Úsala SIEMPRE que el cliente pregunte por precios específicos de dispositivos. Entrada: descripción completa de lo que busca el cliente.",
  func: async (query) => {
    const schema = {
    ...
    if (result.confidence >= 0.8) {
      ...
      response +=
        " Te recomiendo confirmar con el modelo exacto para darte un precio preciso.";
    }

    // Logging para debugging
    logger.info(
      `✅ PRECIO ENCONTRADO: ${result.price} (método: ${result.method}, confianza: ${result.confidence})`,
    );
...
const conversationMemoryTool = new DynamicTool({
  ...
  description:
    "Busca en la memoria de conversaciones pasadas del cliente para personalizar respuestas y recordar interacciones anteriores. Muy útil cuando el cliente dice cosas como 'como la vez pasada', 'el precio que me dijiste', 'mi teléfono', etc. La entrada debe ser la consulta o el ID del cliente.",
  func: async (query) => {
    const schema = {
      required: ["query"],
      types: {
    ...
    const args = { query };
    const validation = validateArguments(
      "consultar_memoria_conversacional",
      args,
      schema,
    );
    if (!validation.isValid) {
    ...
    return await performanceMonitor.measureAsync(
      ...
      async () => {
        ...
        if (phoneMatch) {
          ...
          searchQuery =
            query.replace(phoneMatch[0], "").trim() || "conversación historial";
        }
        ...
        if (clientId) {
          ...
          logger.info(
            `👤 Memoria específica optimizada del cliente ${clientId}: ${result.memories?.length || result.length} recuerdos`,
          );
        } else {
          // Búsqueda semántica general con optimización avanzada
          result = await conversationMemory.searchConversationMemory(
            searchQuery,
            null,
            { limit: 3 },
          );
          logger.info(
            `🔍 Búsqueda semántica optimizada: ${result.memories?.length || result.length} recuerdos`,
          );
        }
        ...
        logger.info(
          `✅ MEMORIA AVANZADA ENCONTRADA: ${memories.length} recuerdos con optimización completa`,
        );
        return response;
      },
...
function getArchitecturalHealth() {
  try {
    ...
    const overallHealth =
      cacheHealth * 0.3 + deterministicHealth * 0.4 + optimizerHealth * 0.3;
