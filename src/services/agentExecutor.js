...
Thought: {agent_scratchpad}`);
    ...
    async initialize() {
        ...
        this.agentExecutor = new AgentExecutor({
            ...
            tools: this.tools,
            verbose: true,
            maxIterations: this.options.maxToolCalls || config.orchestrator.maxToolCalls || 3,
            returnIntermediateSteps: false,
        });

        logger.info('🧠 SalvaCellAgentExecutor inicializado con fallback y rate limiting.');
    }
    ...
    async execute(input) {
        try {
            ...
            try {
                ...
                logger.warn('Error en el LLM primario. Intentando con el fallback...', error);
                this.agentExecutor.agent = this.prompt.pipe(this.fallbackLlm); // Switch to fallback
                try {
                    result = await this.agentExecutor.invoke({ input });
                } catch (fallbackError) {
                    logger.error('Error en el LLM de fallback. No se pudo procesar la solicitud.', fallbackError);
            ...
            if (error.name === 'RateLimiterRes') { // Correct error handling for rate-limiter-flexible
                throw new Error('RATE_LIMIT_EXCEEDED');
            }
...
module.exports = { SalvaCellAgentExecutor, OllamaError, ToolError, ValidationError };