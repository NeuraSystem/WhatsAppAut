...
class LangChainEmbeddingAdapter {
  ...
  async generate(texts) {
    try {
      ...
      logger.error(
        "Error en LangChainEmbeddingAdapter (SemanticRouter):",
        error,
      );
      throw error;
    }
  ...
  try {
    ...
    if (
      ...
      if (
        ...
        logger.info(
          `Intención: '${intent}' (confianza: ${(confidence * 100).toFixed(1)}%) para: "${userQuery}"`,
        );

        // Si la confianza es muy baja, usar fallback de keywords
        if (confidence < 0.5) {
          logger.warn(
            `Confianza baja en clasificación semántica, usando keywords como respaldo`,
          );
      ...
      logger.warn(
        `No se encontraron resultados en ChromaDB para: "${userQuery}"`,
      );
      return determineIntentByKeywords(userQuery);
    }
  } catch (error) {
    ...
    if (
      ...
      logger.error(
        "La colección de intenciones no existe. Usando clasificación por palabras clave.",
      );
    } else if (error.message.includes("Connection")) {
      logger.error(
        "Error de conexión con ChromaDB. Usando clasificación por palabras clave.",
      );
    }
...
function determineIntentByKeywords(userQuery) {
  ...
  for (const [intent, keywords] of Object.entries(intentKeywords)) {
    if (keywords.some((keyword) => query.includes(keyword))) {
      logger.info(
        `Intención detectada por keywords: '${intent}' para: "${userQuery}"`,
      );
      return intent;
    }
  }
  ...
  if (ambiguousPatterns.some((pattern) => pattern.test(query))) {
    logger.info(
      `Mensaje ambiguo detectado que necesita clarificación: "${userQuery}"`,
    );
  ...
  logger.info(
    `No se pudo clasificar la intención por keywords para: "${userQuery}". Usando 'otro'.`,
  );
  return "otro";
}
