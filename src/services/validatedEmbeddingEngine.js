...
class ValidatedEmbeddingEngine extends EnhancedEmbeddingEngine {
  ...
  async initializeDimensions() {
    try {
      ...
      const testEmbedding = await super.embedDocument(
        "test embedding initialization",
      );

      if (!testEmbedding || !Array.isArray(testEmbedding)) {
        throw new Error(
          "Failed to generate test embedding for dimension detection",
        );
      }
      ...
      logger.info(
        `✅ Dimensiones detectadas: ${this.config.expectedDimensions}D`,
      );
      return true;
    } catch (error) {
  ...
  async embedDocument(text, context = "CONVERSATION_EMBEDDINGS") {
    ...
    try {
      ...
      // Validar embedding
      const validationResult = await this.validateEmbedding(
        embedding,
        context,
        text,
      );
      ...
      if (!validationResult.isValid) {
        logger.warn(
          `⚠️ Embedding inválido para documento: ${validationResult.errors.join(", ")}`,
        );
        ...
        if (recoveredEmbedding) {
          ...
          throw new Error(
            `Failed to generate valid embedding: ${validationResult.errors.join(", ")}`,
          );
        }
      }
  ...
  async embedQuery(text, context = "PRICE_QUERY_EMBEDDINGS") {
    ...
    try {
      ...
      if (!validationResult.isValid) {
        logger.warn(
          `⚠️ Embedding de consulta inválido: ${validationResult.errors.join(", ")}`,
        );
        ...
        if (recoveredEmbedding) {
          ...
          throw new Error(
            `Failed to generate valid query embedding: ${validationResult.errors.join(", ")}`,
          );
        }
      }
  ...
  async embedDocuments(texts, context = "CONVERSATION_EMBEDDINGS") {
    ...
    try {
      ...
      if (recoveryNeeded.length > 0) {
        logger.warn(
          `🔄 Recuperando ${recoveryNeeded.length} embeddings inválidos...`,
        );
        ...
        for (const recovery of recoveryNeeded) {
          try {
            ...
            if (recoveredEmbedding) {
              ...
              throw new Error(
                `Failed to recover embedding for: ${recovery.text.substring(0, 50)}...`,
              );
            }
          } catch (recoveryError) {
            logger.error(
              `❌ Fallo en recuperación para índice ${recovery.index}:`,
              recoveryError,
            );
            throw recoveryError;
          }
      ...
      logger.info(
        `✅ Validación en lote completada: ${texts.length} embeddings, ${recoveryNeeded.length} recuperados`,
      );
   ...
   */
  async validateEmbedding(
    embedding,
    context = "CONVERSATION_EMBEDDINGS",
    originalText = "",
  ) {
    ...
    try {
      ...
      if (!embedding || !Array.isArray(embedding)) {
        ...
        validationResult.errors.push(
          "Embedding is null, undefined, or not an array",
        );
      ...
      const valueRange =
        this.config.valueRanges[context] ||
        this.config.valueRanges.CONVERSATION_EMBEDDINGS;
      const outOfRangeValues = embedding.filter(
        (val) => val < valueRange.min || val > valueRange.max,
      );

      if (outOfRangeValues.length > 0) {
        const percentage = (
          (outOfRangeValues.length / embedding.length) *
          100
        ).toFixed(1);

        if (percentage > 10) {
          // Más del 10% fuera de rango es crítico
          validationResult.isValid = false;
          validationResult.errors.push(
      ...
      const qualityScore = this.calculateEmbeddingQuality(
        embedding,
        context,
        originalText,
      );
      ...
      if (context === "MARKDOWN_CONTEXT_EMBEDDINGS") {
        const markdownValidation = this.validateMarkdownEmbedding(
          embedding,
          originalText,
        );
  ...
  detectSalvaCellContext(text) {
    ...
    if (
      this.containsAny(lowerText, ["precio", "costo", "cuánto", "$", "pesos"])
    ) {
  ...
  validateMarkdownEmbedding(embedding, originalText) {
    ...
    if (
      this.containsAny(lowerText, ["tiempo", "minutos", "horas", "domicilio"])
    ) {
      const timeConsistency = this.checkTimeConsistency(embedding);
      if (timeConsistency < 0.8) {
        result.warnings.push(
          `Time-related embedding consistency low: ${timeConsistency.toFixed(3)}`,
        );
      }
    }
    ...
    if (this.containsAny(lowerText, ["garantía", "original", "genérica"])) {
      ...
      if (warrantyConsistency < 0.8) {
        result.warnings.push(
          `Warranty-related embedding consistency low: ${warrantyConsistency.toFixed(3)}`,
        );
      }
    }
    ...
    if (
      this.containsAny(lowerText, ["peso", "mxn", "$"]) &&
      this.containsAny(lowerText, [
        "800",
        "1000",
        "1500",
      ...
      if (priceConsistency < 0.8) {
        result.warnings.push(
          `Price-related embedding consistency low: ${priceConsistency.toFixed(3)}`,
        );
      }
    }
  ...
  calculateEmbeddingQuality(embedding, context, originalText) {
    ...
    const salvaCellRelevance = this.checkSalvaCellRelevance(
      embedding,
      originalText,
    );
  ...
  async attemptRecovery(originalText, validationResult, context) {
    ...
    try {
      ...
      if (
        validationResult.errors.some((err) =>
          err.includes("Dimension mismatch"),
        )
      ) {
        ...
        const revalidation = await this.validateEmbedding(
          recoveredEmbedding,
          context,
          originalText,
        );
      ...
      if (
        validationResult.errors.some((err) => err.includes("invalid values"))
      ) {
      ...
      const fallbackEmbedding = await super.embedDocument(
        `[Contexto SalvaCell] ${originalText}`,
      );
      const finalValidation = await this.validateEmbedding(
        fallbackEmbedding,
  ...
  checkSalvaCellRelevance(embedding, originalText) {
    ...
    const hasRelevantKeywords = this.containsAny(
  ...
  recordValidation(success, timeMs, context, result, batchSize = 1) {
    ...
    const totalTime =
      ...
      timeMs;
    this.validationStats.averageValidationTime =
      totalTime / this.validationStats.totalValidations;

    // Verificar umbrales de alerta
    this.checkAlertThresholds();

    logger.debug(
      `📊 Validación registrada: ${context} - ${result} (${timeMs}ms)`,
    );
  }
  ...
  checkAlertThresholds() {
    ...
    const recoveryRate =
      this.validationStats.recoveryAttempts > 0
        ? this.validationStats.successfulRecoveries /
          this.validationStats.recoveryAttempts
        : 1;
    ...
    if (failureRate > this.alertThresholds.criticalFailureRate) {
      ...
      this.triggerAlert(
        "WARNING",
        `Elevated failure rate: ${(failureRate * 100).toFixed(1)}%`,
      );
    }
    ...
    if (recoveryRate < 1 - this.alertThresholds.recoveryFailureRate) {
      this.triggerAlert(
        "CRITICAL",
        `Low recovery success rate: ${(recoveryRate * 100).toFixed(1)}%`,
      );
    }
  }
  ...
  getValidationStats() {
    ...
    return {
      ...
      status:
        failureRate < this.alertThresholds.warningFailureRate
          ? "healthy"
          : "degraded",
      expectedDimensions: this.config.expectedDimensions,
