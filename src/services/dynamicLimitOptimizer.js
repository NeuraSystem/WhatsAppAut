...
class DynamicLimitOptimizer {
   ...
   */
    optimizeLimits(query, currentContext = 'DEFAULT', currentFilters = {}) {
        ...
        this.detectQueryContext(query, currentFilters) || currentContext;
            ...
            const safeLimits = this.applySafetyConstraints(
                optimizedLimits,
                finalContext
            );
            ...
            logger.debug(
                `🎯 Límites optimizados: ${query.substring(0, 50)}... → Contexto: ${finalContext}, Base: ${safeLimits.baseLimit}, Max: ${safeLimits.maxLimit}`
            );
   ...
   */
    analyzeQueryComplexity(query) {
        ...
        Object.entries(this.config.complexityAnalysis).forEach(
            ([type, patterns]) => {
                let typeMatches = 0;
        ...
        const brandMentions = (
            lowerQuery.match(
                /\b(?:samsung|iphone|xiaomi|motorola|lg|huawei|oppo|vivo|realme)\b/g
            ) || []
        ).length;
   ...
   */
    isMultiDeviceQuery(query) {
        ...
        const hasMultiPatterns = multiPatterns.some((pattern) =>
   ...
   */
    calculateOptimalLimits(context, complexityAnalysis, filters) {
        const contextConfig =
      this.config.contextLimits[context] || this.config.contextLimits.DEFAULT;
        ...
        const maxAbsolute = Math.min(
            100,
            Math.round(this.config.salvaCellConfig.catalogSize * 0.3)
        ); // Max 30% del catálogo
   ...
   */
    applySafetyConstraints(limits, context) {
      ...
      this.config.salvaCellConfig.maxResponseTime -
      this.config.salvaCellConfig.performanceBuffer;
        ...
        if (limits.expectedTime > maxAllowedTime) {
            ...
            limits.baseLimit = Math.max(
                5,
                Math.round(limits.baseLimit * reductionFactor)
            );
            limits.maxLimit = Math.max(
                limits.baseLimit,
                Math.round(limits.maxLimit * reductionFactor)
            );
            limits.expectedTime = Math.round(limits.expectedTime * reductionFactor);
            limits.strategy = 'performance_constrained';

            logger.warn(
                `⚠️ Límites reducidos por restricciones de performance: ${context}`
            );
        }
   ...
   */
    ...
    // Tiempo base por resultado (basado en métricas reales)
        ...
        const estimatedTime =
      (baseTime + networkLatency + processingOverhead) * multiplier;
   ...
   */
    recordOptimization(_context, complexity, limits, duration) {
        if (!this.metrics.contextBreakdown[_context]) {
            this.metrics.contextBreakdown[_context] = {
                count: 0,
                avgComplexity: 0,
                avgBaseLimit: 0,
                avgMaxLimit: 0,
        ...
        const contextMetrics = this.metrics.contextBreakdown[_context];
        contextMetrics.count++;

        // Actualizar promedios
        contextMetrics.avgComplexity = this.updateAverage(
            contextMetrics.avgComplexity,
            complexity.score,
            contextMetrics.count
        );
        contextMetrics.avgBaseLimit = this.updateAverage(
            contextMetrics.avgBaseLimit,
            limits.baseLimit,
            contextMetrics.count
        );
        contextMetrics.avgMaxLimit = this.updateAverage(
            contextMetrics.avgMaxLimit,
            limits.maxLimit,
            contextMetrics.count
        );
        contextMetrics.avgDuration = this.updateAverage(
   ...
   */
    ...
    getContextHealth(context) {
        ...
        const avgEfficiency =
      contextMetrics.avgDuration < contextConfig.timeoutMs
          ? 1.0
          : contextConfig.timeoutMs / contextMetrics.avgDuration;
    ...
    calculateSystemHealth() {
      ...
      this.metrics.totalOptimizations;
        const timeoutRate =
      this.metrics.performanceImpact.timeoutCount /
      this.metrics.totalOptimizations;
        ...
        const reliabilityScore = Math.max(
            0,
            1 - fallbackRate * 2 - timeoutRate * 3
        );
