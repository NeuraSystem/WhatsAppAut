...
class MarkdownMetadataExtractor {
    constructor(options = {}) {
        this.config = {
            ...
            markdownDir: options.markdownDir || path.join(process.cwd(), 'data', 'processed_for_ai', 'precios_markdown'),
    ...
    extractBasicInfoFromContent(content, filePath) {
        try {
            ...
            const tableLines = lines.filter(line => line.includes('|') && !line.includes('Modelo'));
            ...
            logger.warn(`Error extrayendo información básica de ${filePath}:`, error.message);
            return null;
        }
    }
    ...
    async healthCheck() {
        try {
            ...
            if (cacheStats.hit_rate < 0.5 && cacheStats.hits + cacheStats.misses > 10) {
                health.status = 'degraded';
            ...
            try {
                ...
                health.issues.push('No se puede acceder al directorio de archivos Markdown');
            }
