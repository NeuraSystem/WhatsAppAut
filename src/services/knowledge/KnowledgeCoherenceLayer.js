...
class KnowledgeCoherenceLayer extends EventEmitter {
    constructor() {
        ...
        logger.info('🧬 Enhanced KnowledgeCoherenceLayer initialized', {
            features: ['cross-source-validation', 'real-time-monitoring', 'auto-conflict-resolution'],
            config: this.config
        });
    }
    ...
    async validateSearchResults(searchResult, originalQuery, context) {
        ...
        try {
            ...
            validation.checks.freshness = await this.validateDataFreshness(searchResult);
            ...
            validation.checks.businessLogic = await this.validateBusinessLogic(searchResult);
    ...
    async validateBusinessLogic(searchResult) {
        try {
            ...
            if (!isBusinessHours && searchResult.metadata.searchType === 'PRICE_SPECIFIC') {
                businessValidation.issues.push({
                    issue: 'after_hours_pricing_query',
                    details: 'Price queries after business hours may have stale information'
                });
    ...
    async performHealthCheck() {
        try {
            ...
            health.components.crossValidation = await this.checkCrossValidationHealth();
    ...
    async calculateSemanticRelevance(result, query) {
        try {
            ...
            const matchedWords = queryWords.filter(word => 
                word.length > 2 && resultLower.includes(word)
            );
    ...
    async detectSemanticContradictions(results) {
        ...
        try {
            ...
            if (priceResults.length > 1) {
                ...
                if (maxPrice / minPrice > 2) { // 100% price variation
                    contradictions.push({
            ...
            const availabilityResults = results.filter(r => r.availability !== undefined);
            const availableCount = availabilityResults.filter(r => r.availability).length;
            ...
            logger.warn('⚠️ Error detecting contradictions', { error: error.message });
        }
    ...
    validatePriceRange(price) {
        ...
        if (numericPrice > 100000) { // Assuming max reasonable price
    ...
    async checkPostgreSQLFreshness() {
        try {
            ...
            logger.warn('⚠️ Unable to check PostgreSQL freshness', { error: error.message });
    ...
    async performHealthCheck() {
        try {
            ...
            healthCheck.components.embeddings = await this.checkEmbeddingEngineHealth();
            
            // Check cross-validator health
            healthCheck.components.crossValidator = await this.crossSourceValidator.healthCheck();
            
            // Calculate overall health score
            const componentScores = Object.values(healthCheck.components)
                .map(component => component.score || 0.5);
            
            healthCheck.score = componentScores.reduce((a, b) => a + b, 0) / componentScores.length;
    ...
    updateCoherenceHistory(score) {
        ...
        // Update average
        const sum = this.coherenceHistory.reduce((acc, entry) => acc + entry.score, 0);
    ...
    getEnhancedMetrics() {
        ...
        return {
            ...
            validationRate: this.metrics.crossValidations / Math.max(1, this.metrics.totalSearches),
            conflictRate: this.metrics.conflictsDetected / Math.max(1, this.metrics.crossValidations),
            resolutionRate: this.metrics.conflictsResolved / Math.max(1, this.metrics.conflictsDetected),
            coherenceTrend: this.analyzeCoherenceTrend(),
            lastHealthCheck: this.metrics.lastHealthCheck,
            isHealthy: this.metrics.averageCoherenceScore > this.config.coherenceThreshold
        };
    }
    ...
    analyzeCoherenceTrend() {
        ...
        const trend = change > 0.05 ? 'improving' : change < -0.05 ? 'degrading' : 'stable';
    ...
    async search(query, context = {}) {
        ...
        switch (searchStrategy.type) {
            case 'PRICE_SPECIFIC':
                return await this.executePriceSpecificSearch(query, context, searchStrategy);
    ...
    async determineOptimalStrategy(query, context) {
        ...
        if (queryAnalysis.containsPriceKeywords && queryAnalysis.hasSpecificDevice) {
            return {
                type: 'PRICE_SPECIFIC',
                confidence: 0.9,
        ...
        if (queryAnalysis.isGeneralInformation && !queryAnalysis.containsPriceKeywords) {
            return {
    ...
    async executeSemanticSearch(query, context, strategy) {
        try {
            const semanticResults = await this.chromaConnector.semanticSearch(
                query,
                {
                    limit: 8,
    ...
    async executeHybridSearch(query, context, strategy) {
        try {
            ...
            const [priceResults, semanticResults] = await Promise.allSettled([
                ...
                this.chromaConnector.semanticSearch(
                    query,
                    { 
                        limit: 5,
                        timeout: 3000,
        ...
        for (const result of semanticResults) {
            ...
            if (this.mentionsPricing(result.content)) {
                ...
                if (priceValidation.hasInconsistency) {
                    logger.warn(`Price inconsistency detected in semantic result: ${result.id}`);
        ...
        const enrichmentPromises = priceResults.map(async (priceResult) => {
            try {
                ...
                const relatedContext = await this.chromaConnector.findRelatedContext(
                    ...
                    {
                        limit: 3,
                        filters: { topic: ['warranties', 'installation', 'recommendations'] }
                    }
                );
                ...
                logger.warn(`Failed to enrich price result for ${priceResult.device_model}:`, error);
                return priceResult; // Devolver sin enriquecimiento
            }
        });

        return await Promise.allSettled(enrichmentPromises)
    ...
    detectPriceKeywords(query) {
        const priceKeywords = ['precio', 'costo', 'cuanto', 'tarifa', 'vale', 'cobran', '$'];
    ...
    detectGeneralInformation(query) {
        const generalKeywords = ['ubicacion', 'direccion', 'horario', 'telefono', 'contacto', 'como llegar'];
    ...
    async classifyIntent(query) {
        try {
            if (embeddingEngine && embeddingEngine.embedClassification) {
                const categories = ['consulta_precio', 'consulta_ubicacion', 'consulta_disponibilidad', 'conversacion_general'];
                const embedding = await embeddingEngine.embedClassification(query, categories);
...
class CoherenceEngine {
    fuseMultiSourceResults(results, context) {
        ...
        if (results.priceResults && results.priceResults.length > 0) {
            fused.push(...results.priceResults.map(r => ({
                ...r,
                source: 'postgresql',
                type: 'price'
            })));
        ...
        if (results.semanticResults && results.semanticResults.length > 0) {
            fused.push(...results.semanticResults.map(r => ({
                ...r,
                source: 'chromadb',
                type: 'semantic'
            })));
