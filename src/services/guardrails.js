...
class Guardrails {
    ...
    // Las reglas ahora usan la configuración externa
        ...
        logger.info(
            '🛡️ Guardrails inicializados con reglas de validación desde la configuración central.'
        );
    }
    ...
    processResponse(agentResponse, userQuery) {
        ...
        for (const rule of this.rules) {
            ...
            if (result.violation) {
                ...
                if (result.critical) {
                    logger.warn(
                        `CRITICAL GUARDRAIL VIOLATION: ${result.message} for query: "${userQuery}"`
                    );
        ...
        if (violations.length > 0) {
            logger.warn(
                `Guardrail violations detected for query: "${userQuery}": ${violations.join('; ')}`
            );
   ...
   */
    checkNoPromises(response) {
        if (
            config.guardrails.noPromises.some((keyword) => keyword.test(response))
        ) {
            return {
                violation: true,
                message: 'Respuesta contiene promesas no garantizadas.',
                critical: false
            };
        }
   ...
   */
    checkNoToxicity(response) {
        if (
            config.guardrails.noToxicity.some((keyword) => keyword.test(response))
        ) {
            return {
                violation: true,
                message: 'Respuesta contiene lenguaje tóxico.',
                critical: true
            };
        }
        return { violation: false };
   ...
   */
    ...
    // Lógica simple mejorada para detectar si el tema es de negocio o no
        const isBusinessRelated = config.guardrails.businessKeywords.some(
            (keyword) => keyword.test(response) || keyword.test(userQuery)
        );
        const isClearlyOffTopic = config.guardrails.offTopicKeywords.some(
            (keyword) => keyword.test(response)
        );

        if (isClearlyOffTopic && !isBusinessRelated) {
            return {
                violation: true,
                message: 'Respuesta fuera de tema.',
                critical: false
            };
        }
        return { violation: false };
   ...
   */
    checkNoPersonalOpinions(response) {
        if (
            config.guardrails.noPersonalOpinions.some((keyword) =>
                keyword.test(response)
            )
        ) {
            return {
                violation: true,
                message: 'Respuesta contiene opinión personal.',
                critical: false
            };
        }
        return { violation: false };
   ...
   */
    checkNoSensitiveInfoRequest(response) {
        if (
            config.guardrails.noSensitiveInfoRequest.some((keyword) =>
                keyword.test(response)
            )
        ) {
            return {
                violation: true,
                message: 'Respuesta solicita información sensible.',
                critical: true
            };
        }
        return { violation: false };
   ...
   */
    checkResponseLength(response) {
        ...
        if (response.length > maxLength) {
            return {
                violation: true,
                message: `Respuesta excede la longitud máxima de ${maxLength} caracteres.`,
                critical: false
            };
        }
        return { violation: false };
