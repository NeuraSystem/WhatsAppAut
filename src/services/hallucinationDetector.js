...
const { responseValidatorPipeline } = require('../utils/validators');
...
async function detectHallucination(response, context = {}) {
    ...
    const validation = await responseValidatorPipeline.validate(
        response,
        context
    );
    ...
    return {
        isHallucination:
      validation.riskLevel === 'high' || validation.riskLevel === 'critical',
