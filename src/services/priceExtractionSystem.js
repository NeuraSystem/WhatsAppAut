...
const config = require('../utils/config'); // Corregido: require, no antrp
...
class PriceExtractionSystem {
    ...
    async extractPrice(query) {
        ...
        if (!extracted.device) {
            logger.warn('No se pudo extraer el dispositivo con el LLM. Terminando extracción.');
            return {
                price: null,
                confidence: 0,
                method: 'no_device_extracted',
                message: 'No pude identificar el modelo del dispositivo en tu pregunta. ¿Podrías ser más específico?',
        ...
        for (let i = 0; i < this.strategies.length; i++) {
            try {
                ...
                if (result && this.validateResult(result)) {
                    ...
                    try {
                        coherenceValidation = await coherenceLayer.validateBusinessLogic({
                            results: [{ id: `${extracted.device}_${extracted.service}`, price: result.price }],
                    ...
                    try {
                        temporalValidation = await coherenceLayer.validateDataFreshness({ metadata: { sources: ['postgresql'] } });
                    } catch (e) {
                ...
                logger.warn(`⚠️ Estrategia ${this.getStrategyName(i)} falló:`, error.message);
                continue;
            }
        }
        ...
        if (result.rows.length > 0) {
            ...
            if (similarDevices.length > 0) {
                const avgPrice = Math.round(
                    similarDevices.reduce((sum, row) => sum + (Number(row.precio) || 0), 0) / similarDevices.length
                ) || 0;
                return {
                    price: avgPrice,
    ...
    validateResult(result) {
        if (!result || result.price === null || result.price === undefined) return false;
        const price = Number(result.price);
        if (isNaN(price) || price <= 0) return false;
        if (price < config.priceExtraction.minPriceValidation || price > config.priceExtraction.maxPriceValidation) return false;
        if (result.confidence < config.priceExtraction.minConfidenceValidation) return false;
        return true;
    }
}
