...
class SimpleDeduplicationEngine {
    ...
    async checkForDuplicates(newChunk, existingChunks = []) {
        ...
        try {
            ...
            const decision = this.makeConservativeDecision(duplicateAnalysis, newChunk);

            // 6. Registrar métricas
            this.recordMetrics(decision, infoType, Date.now() - startTime);

            logger.debug(`🔍 Deduplicación: ${newChunk.text.substring(0, 50)}... → ${decision.action} (${infoType}, similitud: ${duplicateAnalysis.maxSimilarity?.toFixed(3) || 'N/A'})`);
    ...
    classifyInformationType(chunk) {
        ...
        if (this.containsPatterns(text, ['precio', 'costo', '$', 'vale', 'cobran']) || 
            metadata.price_quoted) {
            return 'PRICE_INFORMATION';
        }

        if (this.containsPatterns(text, ['garantía', 'warranty', '30 días', '15 días']) ||
            metadata.main_intent === 'warranty_query') {
            return 'WARRANTY_INFORMATION';
        }

        if (this.containsPatterns(text, ['samsung', 'iphone', 'xiaomi', 'motorola', 'lg', 'huawei']) ||
            metadata.device_mentioned) {
            return 'DEVICE_INFORMATION';
        }

        if (this.containsPatterns(text, ['pantalla', 'batería', 'cámara', 'puerto', 'reparación']) ||
     ...
     */
    async findDuplicatesWithContext(newChunk, existingChunks, threshold, infoType) {
        const duplicates = [];
        let maxSimilarity = 0;

        // Obtener contexto Markdown relevante
        const markdownContext = await this.getRelevantMarkdownContext(newChunk, infoType);
        ...
        for (const existingChunk of existingChunks) {
            try {
                ...
                logger.warn(`⚠️ Error calculando similitud con chunk ${existingChunk.id}:`, error.message);
            }
        }
     ...
     */
    async calculateContextualSimilarity(newChunk, existingChunk, markdownContext, infoType) {
        ...
        const textSimilarity = this.calculateJaccardSimilarity(newChunk.text, existingChunk.text);
        ...
        switch (infoType) {
            case 'PRICE_INFORMATION':
                if (metadata1.device_mentioned && metadata2.device_mentioned) {
                    const deviceSim = this.calculateDeviceSimilarity(metadata1.device_mentioned, metadata2.device_mentioned);
                    similarity += deviceSim;
                    factors++;
                }
                if (metadata1.price_quoted && metadata2.price_quoted) {
                    const priceDiff = Math.abs(parseFloat(metadata1.price_quoted) - parseFloat(metadata2.price_quoted));
            ...
            case 'SERVICE_DESCRIPTIONS':
                if (metadata1.service_mentioned && metadata2.service_mentioned) {
                    similarity += metadata1.service_mentioned === metadata2.service_mentioned ? 1 : 0;
                    factors++;
                }
                break;
        }
    ...
    calculateMarkdownContextSimilarity(newChunk, existingChunk, markdownContext) {
        ...
        if (markdownContext.timeInfo) {
            const newHasTime = this.containsPatterns(newChunk.text, ['tiempo', 'minutos', 'horas', '4 pm']);
            const existingHasTime = this.containsPatterns(existingChunk.text, ['tiempo', 'minutos', 'horas', '4 pm']);
        ...
        if (markdownContext.warrantyInfo) {
            const newHasWarranty = this.containsPatterns(newChunk.text, ['garantía', '30 días', '15 días']);
            const existingHasWarranty = this.containsPatterns(existingChunk.text, ['garantía', '30 días', '15 días']);
    ...
    getThresholdForType(infoType) {
        return this.config.typeThresholds[infoType] || this.config.similarityThreshold;
    }
    ...
    getDuplicateReason(similarity, infoType) {
        ...
        if (similarity >= 0.90) return 'similar';
    ...
    async getRelevantMarkdownContext(chunk, infoType) {
        try {
            ...
            const enriched = this.markdownEnricher.enrichSearchResults(mockResults, chunk.text, infoType);
    ...
    recordMetrics(decision, infoType, duration) {
        ...
        typeStats.avgDuration = (typeStats.avgDuration * (typeStats.checked - 1) + duration) / typeStats.checked;
    }
    ...
    getDeduplicationStats() {
        const totalProcessed = this.metrics.duplicatesFound + this.metrics.informationPreserved;
        ...
        return {
            ...
            duplicateRate: totalProcessed > 0 ? (this.metrics.duplicatesFound / totalProcessed) : 0,
            preservationRate: totalProcessed > 0 ? (this.metrics.informationPreserved / totalProcessed) : 0,
    ...
    calculateSystemHealth() {
        ...
        const totalProcessed = this.metrics.duplicatesFound + this.metrics.informationPreserved;
        const preservationRate = totalProcessed > 0 ? (this.metrics.informationPreserved / totalProcessed) : 0.9;
        const duplicateRate = totalProcessed > 0 ? (this.metrics.duplicatesFound / totalProcessed) : 0;
        ...
        const overallHealth = (preservationScore * 0.5) + (efficiencyScore * 0.3) + (performanceScore * 0.2);
