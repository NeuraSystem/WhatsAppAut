...
class MarkdownContextEnricher {
    ...
    async refreshMarkdownCache() {
        try {
            ...
            try {
                ...
                logger.warn(
                    `📁 Directorio Markdown no encontrado: ${this.config.markdownPath}`
                );
                return;
            }
            ...
            logger.info(
                `✅ Cache Markdown actualizado: ${updatedCount} archivos procesados en ${duration}ms`
            );
        } catch (error) {
   ...
   */
    buildGlobalMetadata() {
        ...
        const allModels = Array.from(this.cache.brandInfo.values()).flatMap(
            (brandInfo) => brandInfo.models
        );
   ...
   */
    enrichSearchResults(results, query, context = 'general') {
        ...
        try {
            ...
            results.globalMarkdownInfo = this.getRelevantGlobalInfo(
                enrichmentType,
                relevantBrands
            );

            logger.debug(
                `📄 Resultados enriquecidos con contexto Markdown: ${enrichmentType.join(', ')}`
            );
        } catch (error) {
            logger.warn(
                '⚠️ Error enriqueciendo resultados con Markdown:',
                error.message
            );
        }
   ...
   */
    extractBrandsFromQuery(query) {
        ...
        this.cache.brandInfo.forEach((brandInfo, brandKey) => {
            if (
                lowerQuery.includes(brandKey) ||
        lowerQuery.includes(brandInfo.brand.toLowerCase())
            ) {
