...
class SemanticChunker {
    constructor(options = {}) {
        ...
        this.config = {
            ...
            markdownKeywords: ['establecimiento', 'domicilio', '4 PM', 'garantía', 'original', 'genérica'],
     ...
     */
    async createSemanticChunks(clientId, userMessage, botResponse, intent, extractedData = {}) {
        try {
            // 1. Crear entrada de conversación actual
            const conversationEntry = this.createConversationEntry(
                clientId, userMessage, botResponse, intent, extractedData
            );
            ...
            const chunks = await this.createSlidingWindowChunks(
                clientId, conversationEntry, windowConfig
            );
            ...
            return [await this.createFallbackChunk(clientId, userMessage, botResponse, intent, extractedData)];
        }
    }
    ...
    async createSlidingWindowChunks(clientId, currentEntry, windowConfig) {
        ...
        if (conversations.length < windowSize) {
            const chunk = await this.createContextualChunk(
                conversations, conversations.length - 1, currentEntry, 'simple'
            );
            chunks.push(chunk);
            return chunks;
        }
        ...
        for (let i = 0; i < conversations.length; i++) {
            ...
            const windowEnd = Math.min(conversations.length, windowStart + windowSize);
            ...
            const chunk = await this.createContextualChunk(
                contextWindow, focusIndex, currentEntry, 'windowed'
            );
     ...
     */
    async createContextualChunk(contextWindow, focusIndex, currentEntry, type = 'windowed') {
        ...
        const metadata = this.buildEnhancedMetadata(
            primaryConversation, contextWindow, currentEntry, type
        );
    ...
    extractMarkdownContext(conversation) {
        ...
        if (this.containsAny(userMessage + ' ' + botResponse, ['tiempo', 'cuánto', 'demora', 'domicilio', 'establecimiento'])) {
            context += '• Tiempo establecimiento: Mismo día (antes 4PM) / Siguiente día (después 4PM)\n';
            context += '• Tiempo domicilio: 45-60 minutos una vez iniciado\n';
        }
        
        // Detectar si se mencionan temas relacionados con garantías
        if (this.containsAny(userMessage + ' ' + botResponse, ['garantía', 'original', 'genérica', 'calidad'])) {
            context += '• Garantía original: 30 días\n';
            context += '• Garantía genérica: 15 días\n';
        }
        
        // Detectar si se mencionan precios específicos
        if (extractedData.device && (intent === 'consulta_precio' || botResponse.includes('$'))) {
     ...
     */
    buildEnhancedMetadata(primaryConversation, contextWindow, currentEntry, type) {
        ...
        return {
            ...
            hour_of_day: this.getHourCategory(new Date(primaryConversation.timestamp)),
    ...
    analyzeConversationFlow(contextWindow) {
        ...
        const devices = contextWindow.map(conv => conv.extractedData.device).filter(Boolean);
        const prices = contextWindow.map(conv => conv.extractedData.price).filter(Boolean);
        
        // Detectar patrones específicos de SalvaCell
        if (intents.includes('consulta_precio') && intents.includes('consulta_disponibilidad')) {
    ...
    calculateSemanticDensity(contextWindow) {
        ...
        const extractedInfo = contextWindow.reduce((acc, conv) => {
            if (conv.extractedData.device) acc.devices.add(conv.extractedData.device);
            if (conv.extractedData.service) acc.services.add(conv.extractedData.service);
            if (conv.extractedData.price) acc.prices.add(conv.extractedData.price);
            return acc;
        }, { devices: new Set(), services: new Set(), prices: new Set() });
        
        const infoRichness = (extractedInfo.devices.size + extractedInfo.services.size + extractedInfo.prices.size) / (contextWindow.length * 3);
        score += infoRichness;
        factors++;
        
        // Factor 3: Longitud promedio de mensajes
        const avgLength = contextWindow.reduce((sum, conv) => sum + conv.userMessage.length, 0) / contextWindow.length;
    ...
    calculateContinuityScore(contextWindow) {
        ...
        for (let i = 1; i < contextWindow.length; i++) {
            ...
            if (prev.extractedData.device === curr.extractedData.device && prev.extractedData.device) {
                continuityScore += 0.3;
            }
            
            // Verificar continuidad de servicio
            if (prev.extractedData.service === curr.extractedData.service && prev.extractedData.service) {
                continuityScore += 0.2;
            }
    ...
    createConversationEntry(clientId, userMessage, botResponse, intent, extractedData) {
        return {
            clientId,
            userMessage,
            botResponse,
    ...
    async createFallbackChunk(clientId, userMessage, botResponse, intent, extractedData) {
        return {
    ...
    classifyResponseType(response) {
        if (response.includes('$') || response.includes('precio')) return 'price_quote';
        if (response.includes('minutos') || response.includes('tiempo')) return 'time_estimate';
        if (response.includes('disponible')) return 'availability';
        if (response.includes('Hola') || response.includes('soy Sofia')) return 'greeting';
        if (response.includes('gracias') || response.includes('Perfecto')) return 'closing';
        return 'information';
    }
    ...
    getChunkerStats() {
        ...
        const totalConversations = Array.from(this.conversationBuffer.values())
            .reduce((sum, buffer) => sum + buffer.length, 0);
        
        return {
            total_clients: totalClients,
            total_conversations: totalConversations,
            average_conversations_per_client: totalClients > 0 ? totalConversations / totalClients : 0,
...
class ConversationPatternAnalyzer {
    constructor() {
        this.patterns = {
            PRICE_INQUIRY_FLOW: ['saludo', 'consulta_precio', 'consulta_disponibilidad'],
            WARRANTY_CLAIM_FLOW: ['queja', 'escalar_a_humano'],
            MULTI_DEVICE_FAMILY: ['consulta_precio', 'consulta_precio', 'consulta_precio'],
    ...
    detectPattern(conversations) {
        ...
        const uniqueDevices = new Set(conversations.map(conv => conv.extractedData.device).filter(Boolean));
        if (uniqueDevices.size > 1) {
            return 'MULTI_DEVICE_FAMILY';
        }
        
        const hasPriceQuotes = conversations.some(conv => conv.extractedData.price);
        const hasTimeQuestions = conversations.some(conv => 
