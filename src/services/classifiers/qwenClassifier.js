   ...
   */
    async classify(userQuery) {
        if (!(await this.localClient.checkHealth())) {
            logger.warn(
                'QwenClassifier: LLM local no disponible. Saltando clasificación.'
            );
            return {
                intention: 'fallback',
                confidence: 0,
                rawResponse: 'LLM_UNAVAILABLE'
            };
        }
    ...
    parseResponse(rawResponse) {
        try {
            ...
            if (!this.uniqueIntentions.includes(intention)) {
                logger.warn(
                    `QwenClassifier: Intención clasificada "${intention}" no es una intención válida.`
                );
