...
class DeterministicSearchEngine {
     ...
     */
    async performStabilizedSearch(collection, query, filters = {}, context = 'CONVERSATION_MEMORY') {
        const startTime = Date.now();
        this.metrics.totalQueries++;
        
        try {
            // 1. Detectar contexto específico si no se especifica
            const detectedContext = this.detectQueryContext(query, filters) || context;
            const contextConfig = this.config.contextConfigs[detectedContext] || this.config.contextConfigs.CONVERSATION_MEMORY;
            ...
            if (cacheResult) {
                ...
                this.recordMetrics(detectedContext, 'cache_hit', Date.now() - startTime);
            ...
            const allResults = await this.performMultipleSearches(
                collection, query, filters, contextConfig
            );
            ...
            const stabilizedResults = this.stabilizeResults(
                allResults, filters.limit || 5, contextConfig
            );
            
            // 6. Integrar información Markdown si aplica
            if (contextConfig.markdownIntegration) {
                await this.enrichWithMarkdownContext(stabilizedResults, query, detectedContext);
            }
            
            // 7. Cache para futuras consultas idénticas
            this.cacheResults(queryHash, stabilizedResults, detectedContext, contextConfig.cacheTime);
            
            // 8. Registrar métricas
            this.metrics.stabilizedQueries++;
            this.recordMetrics(detectedContext, 'stabilized', Date.now() - startTime);
            
            logger.debug(`🎯 Búsqueda determinística completada: ${detectedContext} (${Date.now() - startTime}ms)`);
    ...
    async performMultipleSearches(collection, query, filters, contextConfig) {
        ...
        for (let i = 0; i < searchAttempts; i++) {
            const searchPromise = this.performSingleSearch(collection, query, filters, i);
        ...
        try {
            ...
            logger.debug(`🔄 Consenso: ${successfulResults.length}/${searchAttempts} búsquedas exitosas`);
    ...
    stabilizeResults(allResults, targetCount, contextConfig) {
        ...
        const scoredDocuments = Array.from(documentFrequency.entries())
            .map(([docId, frequency]) => {
                ...
                const avgDistance = docData.distances.reduce((sum, dist) => sum + dist, 0) / docData.distances.length;
                const relevanceScore = 1 - avgDistance; // Invertir distancia para score
                
                // Combinar estabilidad y relevancia según configuración
                const combinedScore = (stabilityScore * (1 - relevanceWeight)) + (relevanceScore * relevanceWeight);
        ...
        const avgStabilityScore = scoredDocuments.reduce((sum, doc) => sum + doc.stabilityScore, 0) / scoredDocuments.length;
        ...
        logger.debug(`🎯 Resultados estabilizados: ${scoredDocuments.length}, estabilidad promedio: ${avgStabilityScore.toFixed(3)}`);
    ...
    async enrichWithMarkdownContext(results, query, context) {
        try {
            ...
            const needsTimeInfo = this.containsAny(lowerQuery, this.cacheInvalidation.timePatterns);
            const needsWarrantyInfo = this.containsAny(lowerQuery, this.cacheInvalidation.warrantyPatterns);
            const needsPriceInfo = this.containsAny(lowerQuery, this.cacheInvalidation.pricePatterns);
            ...
            results.metadatas[0].forEach((metadata, index) => {
                ...
                if (needsTimeInfo) {
                    metadata.markdown_enrichment.time_info = {
                        establecimiento: 'Mismo día (antes 4PM) / Siguiente día (después 4PM)',
            ...
            logger.debug(`📄 Enriquecimiento Markdown aplicado: tiempo=${needsTimeInfo}, garantía=${needsWarrantyInfo}, precio=${needsPriceInfo}`);
            
        } catch (error) {
            logger.warn('⚠️ Error enriqueciendo con contexto Markdown:', error.message);
        }
    }
    ...
    detectQueryContext(query, filters = {}) {
        ...
        if (this.containsAny(lowerQuery, ['iphone', 'samsung', 'xiaomi', 'motorola', 'modelo', 'dispositivo'])) {
     ...
     */
    generateQueryHash(query, filters = {}, context = '') {
    ...
    cacheResults(queryHash, results, context, cacheTime) {
        ...
        logger.debug(`💾 Resultado cacheado: ${context} (TTL: ${Math.round(cacheTime/60000)}min)`);
    }

    getCacheByContext(context) {
        switch (context) {
            case 'PRICE_QUERIES': return this.caches.priceCache;
            case 'DEVICE_SEARCH': return this.caches.deviceCache;
            case 'WARRANTY_LOOKUP': return this.caches.warrantyCache;
            case 'MARKDOWN_CONTEXT': return this.caches.markdownCache;
            case 'CONVERSATION_MEMORY': return this.caches.conversationCache;
            default: return this.caches.generalCache;
        }
    }
    ...
    cleanupCache(cache) {
        ...
        if (cache.size >= this.config.maxCacheSize) {
            ...
            const toDelete = sortedEntries.slice(0, Math.floor(this.config.maxCacheSize * 0.2));
    ...
    invalidatePriceCache() {
        ...
        logger.info('🔄 Cache de precios invalidado por actualización de información');
    }
    ...
    recordMetrics(context, operation, duration) {
        ...
        contextMetrics.avgDuration = (contextMetrics.avgDuration * (contextMetrics.queries - 1) + duration) / contextMetrics.queries;
    }
    ...
    getDeterminismStats() {
        ...
        Object.entries(this.caches).forEach(([cacheName, cache]) => {
            const validEntries = Array.from(cache.values()).filter(entry => entry.expiry > now);
        ...
        return {
            ...
            stabilizationRate: this.metrics.totalQueries > 0 ? this.metrics.stabilizedQueries / this.metrics.totalQueries : 0,
            averageStabilityScore: this.metrics.averageStabilityScore,
            consensusFailureRate: this.metrics.totalQueries > 0 ? this.metrics.consensusFailures / this.metrics.totalQueries : 0,
    ...
    getContextHealth(context) {
        ...
        return {
            ...
            cacheHitRate: metrics.queries > 0 ? (metrics.cacheHits || 0) / metrics.queries : 0,
    ...
    calculateContextHealth(context) {
        const metrics = this.metrics.contextBreakdown[context] || {};
        const cacheHitRate = metrics.queries > 0 ? (metrics.cacheHits || 0) / metrics.queries : 0;
