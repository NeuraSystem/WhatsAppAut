...
async function initializeDatabase() {
    try {
        ...
        logger.error('❌ Error al inicializar PostgreSQL:', {
            error: error.message,
            stack: error.stack
        });
...
async function inspectAndNormalizeSchema(client) {
    try {
        ...
        if (tableExists.rows[0].exists) {
            ...
            logger.info(
                '📝 Tabla admin_users no existe, se creará con esquema correcto'
            );
        }
    } catch (error) {
    ...
    // Mapeo de posibles nombres de columnas
        ...
        for (const [standardName, alternatives] of Object.entries(columnMapping)) {
            if (!existingColumns.includes(standardName)) {
                ...
                const existingAlternative = alternatives.find((alt) =>
                    existingColumns.includes(alt)
                );

                if (existingAlternative) {
                    logger.info(
                        `🔄 Normalizando columna: ${existingAlternative} -> ${standardName}`
                    );
                    await client.query(`
...
async function createAdminUsersTableSafely(client) {
    ...
    // Verificar si ya existe
        ...
        logger.error('❌ Error creando tabla admin_users:', {
            error: error.message
        });
        throw error;
    }
...
async function insertDefaultAdminSafely(client) {
    ...
    // Verificar qué columnas existen realmente
      ...
      columns.find((col) =>
        ...
        const nameColumn =
      columns.find((col) => ['name', 'nombre', 'full_name'].includes(col)) ||
      'name';
        ...
        logger.warn('⚠️ No se pudo insertar admin por defecto:', {
            error: error.message
        });
...
async function verifyTables(client) {
    try {
        ...
        try {
            ...
            logger.info(
                `✅ Administradores configurados: ${adminResult.rows[0].count}`
            );
        } catch (error) {
            logger.warn('⚠️ No se pudo verificar administradores:', {
                error: error.message
            });
        }
    } catch (error) {
...
async function addOrUpdateClient(
    ...
    try {
        ...
        logger.error('Error al agregar/actualizar cliente:', {
            error: error.message
        });
    }
}
