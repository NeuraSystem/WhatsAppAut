...
class HealthMonitoringSystem {
  ...
  startMonitoring() {
    if (this.monitorIntervalId) {
      logger.warn(
        "HealthMonitoringSystem: El monitoreo de salud ya está en ejecución.",
      );
      return;
    }
    logger.info(
      "HealthMonitoringSystem: Iniciando monitoreo de salud distribuido...",
    );
  ...
  async runHealthChecks() {
    logger.info(
      "HealthMonitoringSystem: Ejecutando ciclo de chequeos de salud...",
    );
    ...
    for (const serviceName of serviceNames) {
      ...
      if (service && typeof service.healthCheck === "function") {
        try {
          ...
          logger.error(
            `HealthMonitoringSystem: Error durante el chequeo de salud de '${serviceName}'.`,
            error,
          );
        }
      } else {
      ...
      if (!isHealthy) {
        ...
        selfHealingManager.reportFailure(
          serviceName,
          new Error(details.error || "Health check failed"),
        );
      }
    }
    this.analyzeTrends();
  ...
  analyzeTrends() {
    ...
    for (const [serviceName, history] of this.healthHistory.entries()) {
      ...
      if (recentFailures >= 3) {
        logger.warn(
          `HealthMonitoringSystem: El servicio '${serviceName}' muestra una tendencia de fallos recientes.`,
        );
