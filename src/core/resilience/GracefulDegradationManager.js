...
class GracefulDegradationManager extends EventEmitter {
  constructor(options = {}) {
    ...
    this.config = {
      criticalServices: options.criticalServices || [
        "whatsapp",
        "database",
        "ai-processing",
      ],
      importantServices: options.importantServices || [
        "embeddings",
        "cache",
        "logging",
      ],
    ...
    logger.info(
      "🛡️ GracefulDegradationManager inicializado con jerarquía de servicios",
      {
  ...
  evaluateSystemHealth() {
    const healthyServices = Array.from(this.serviceRegistry.values()).filter(
  ...
  getUnhealthyServicesCount(priority) {
    return Array.from(this.serviceRegistry.values()).filter(
      (service) => service.priority === priority && !service.isHealthy,
    ).length;
  }
  ...
  adjustDegradationLevel(systemHealth) {
    ...
    // Decrease degradation if system is recovering
      ...
      systemHealth.unhealthyCritical === 0 &&
