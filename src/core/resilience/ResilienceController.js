...
class ResilienceController {
  ...
  async initialize() {
    if (this.isInitialized) {
      logger.warn(
        "ResilienceController: El controlador de resiliencia ya ha sido inicializado.",
      );
      return true;
    }
    ...
    try {
      logger.info(
        "ResilienceController: Inicializando la capa de resiliencia del sistema...",
      );

      // Initialize circuit breaker
      if (config.resilience?.enableCircuitBreaker) {
        this.advancedCircuitBreaker = new AdvancedCircuitBreaker(
          config.resilience.circuitBreaker,
        );
        logger.info(
          "ResilienceController: ✅ AdvancedCircuitBreaker inicializado",
        );
      }
      ...
      if (config.resilience?.enableGracefulDegradation) {
        this.gracefulDegradationManager = new GracefulDegradationManager(
          config.resilience.gracefulDegradation,
        );
        logger.info(
          "ResilienceController: ✅ GracefulDegradationManager inicializado",
        );
      }
      ...
      logger.info(
        "ResilienceController: Capa de resiliencia inicializada y operativa.",
      );

      return true;
    } catch (error) {
      logger.error(
        "ResilienceController: ❌ Error durante inicialización:",
        error,
      );
      return false;
    }
  ...
  async shutdown() {
    try {
      ...
      if (
        this.advancedCircuitBreaker &&
        typeof this.advancedCircuitBreaker.shutdown === "function"
      ) {
  ...
  async executeWithProtection(operation, context = {}) {
    ...
    if (this.gracefulDegradationManager) {
      ...
      if (systemStatus.degradationLevel > 2) {
        throw new Error(
          `System in critical degradation mode (level ${systemStatus.degradationLevel})`,
        );
      }
    }
  ...
  notifyHealthChange() {
    if (
      ...
      typeof this.onSystemHealthChange === "function"
    ) {
      ...
      if (currentHealth !== this.lastHealthStatus) {
        ...
        this.onSystemHealthChange({
          ...
          degradationLevel: this.gracefulDegradationManager
            ? this.gracefulDegradationManager.getSystemStatus().degradationLevel
            : 0,
          timestamp: Date.now(),
        });
      }
  ...
  getSystemHealth() {
    const health = {
      ...
      circuitBreaker: this.advancedCircuitBreaker
        ? this.advancedCircuitBreaker.getMetrics()
        : null,
      degradation: this.gracefulDegradationManager
        ? this.gracefulDegradationManager.getSystemStatus()
        : null,
    };
    return health;
  }
  ...
  async getStatus() {
    return {
      ...
      circuitBreakerState: this.advancedCircuitBreaker
        ? this.advancedCircuitBreaker.getState()
        : "UNKNOWN",
    };
  }
  ...
  async optimize() {
    ...
    try {
      ...
      if (
        this.advancedCircuitBreaker &&
        this.advancedCircuitBreaker.getState() === "OPEN"
      ) {
      ...
      logger.error(
        "❌ ResilienceController: Error durante optimización:",
        error,
      );
      throw error;
    }
