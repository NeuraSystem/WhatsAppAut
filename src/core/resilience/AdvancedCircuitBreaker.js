...
class AdvancedCircuitBreaker extends EventEmitter {
  constructor(options = {}) {
    ...
    logger.info(
      "🛡️ AdvancedCircuitBreaker inicializado con gestión adaptativa",
      {
        config: this.config,
        state: this.state,
      },
  ...
  async executeWithMonitoring(fn, context, requestId) {
    ...
    try {
      ...
      if (
        this.config.enableResponseTimeCircuit &&
  ...
  shouldAdaptThreshold() {
    ...
    return (
      this.config.enableAdaptiveThreshold &&
  ...
  adaptFailureThreshold() {
    ...
    // Clean old metrics
    this.adaptiveMetrics.recentFailures =
      this.adaptiveMetrics.recentFailures.filter(
        (f) => f.timestamp > windowStart,
      );
    this.adaptiveMetrics.recentSuccesses =
      this.adaptiveMetrics.recentSuccesses.filter(
  ...
  recordResponseTime(responseTime) {
    ...
    const sum = this.responseTimeMetrics.recentResponseTimes.reduce(
    ...
    const healthReport = {
      ...
      successRate:
        this.statistics.totalRequests > 0
          ? `${((this.statistics.totalSuccesses / this.statistics.totalRequests) * 100).toFixed(1)}%`
          : "N/A",
