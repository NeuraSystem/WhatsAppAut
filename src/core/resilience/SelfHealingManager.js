...
class SelfHealingManager {
  ...
  reportFailure(serviceName, error) {
    logger.warn(
      `SelfHealingManager: Falla reportada en el servicio '${serviceName}'. Error: ${error.message}`,
    );
    ...
    if (now - lastAttempt < this.recoveryCooldown) {
      logger.info(
        `SelfHealingManager: El servicio '${serviceName}' está en período de enfriamiento. No se intentará la recuperación.`,
      );
      return;
    }
    ...
    if (attempts > this.maxRecoveryAttempts) {
      logger.error(
        `SelfHealingManager: El servicio '${serviceName}' ha superado el máximo de intentos de recuperación. Se requiere intervención manual.`,
      );
    ...
    logger.info(
      `SelfHealingManager: Iniciando intento de recuperación #${attempts} para el servicio '${serviceName}'.`,
    );
  ...
  async initiateRecovery(serviceName) {
    try {
      ...
      logger.info(
        `SelfHealingManager: Intentando reiniciar el servicio '${serviceName}'...`,
      );
      ...
      if (success) {
        ...
        if (isHealthy) {
          logger.info(
            `SelfHealingManager: El servicio '${serviceName}' se ha recuperado y validado exitosamente.`,
          );
          this.failedServices.delete(serviceName);
          this.recoveryAttempts.delete(serviceName);
        } else {
          logger.error(
            `SelfHealingManager: El servicio '${serviceName}' se reinició pero no pasó la validación de salud.`,
          );
        }
      } else {
        logger.error(
          `SelfHealingManager: Fallo al reiniciar el servicio '${serviceName}'.`,
        );
      }
    } catch (recoveryError) {
      logger.error(
        `SelfHealingManager: Error crítico durante el proceso de recuperación para '${serviceName}'.`,
        recoveryError,
      );
    }
  }
  ...
  async restartService(serviceName) {
    ...
    logger.warn(
      `SelfHealingManager: La función 'restartService' no está implementada en el registro de servicios.`,
    );
    return false;
  }
  ...
  injectDependencies(serviceName) {
    ...
    logger.info(
      `SelfHealingManager: Re-inyectando dependencias para el servicio '${serviceName}' si es necesario.`,
    );
  }
  ...
  async validateRecovery(serviceName) {
    ...
    if (service && typeof service.healthCheck === "function") {
      logger.info(
        `SelfHealingManager: Ejecutando chequeo de salud para '${serviceName}'...`,
      );
      return await service.healthCheck();
    }
    logger.warn(
      `SelfHealingManager: No se encontró un método 'healthCheck' para el servicio '${serviceName}'. Se asume recuperación exitosa.`,
    );
