...
class AdminEscalationSystem {
    ...
    async enqueueEscalation(escalationData, consultaId) {
        ...
        return `📋 Tu consulta ha sido recibida. Hay ${position} persona(s) esperando antes que tú. ` +
    ...
    getCurrentEscalation() {
        ...
        return Array.from(this.activeEscalations.values())
                    .find(esc => esc.status === 'waiting_admin');
    }
    ...
    getMetrics() {
        return {
            ...
            successRate: this.metrics.totalEscalations > 0 ? 
                (this.metrics.resolvedEscalations / this.metrics.totalEscalations) : 0
        };
    }
