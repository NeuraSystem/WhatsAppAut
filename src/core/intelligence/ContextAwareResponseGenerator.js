...
class ContextAwareResponseGenerator {
    ...
    async generateResponse(requestData) {
        ...
        try {
            ...
            const responseType = this.determineResponseType(requestData, clientContext);
            ...
            const conversationHistory = this.getConversationHistory(requestData.clientId);
            ...
            this.updateConversationMemory(requestData.clientId, requestData, response);
    ...
    async updateClientContext(context, requestData) {
        ...
        const communicationStyle = this.analyzeCommunicationStyle(requestData.query);
        if (communicationStyle && communicationStyle !== context.personality.communicationStyle) {
    ...
    determineResponseType(requestData, clientContext) {
        ...
        if (query.includes('precio') || query.includes('costo') || query.includes('cuanto')) {
            return 'pricing_inquiry';
        }
        if (query.includes('horario') || query.includes('hora') || query.includes('abierto')) {
            return 'hours_inquiry';
        }
        if (query.includes('ubicacion') || query.includes('direccion') || query.includes('donde')) {
            return 'location_inquiry';
        }
        if (query.includes('cita') || query.includes('agendar') || query.includes('reservar')) {
            return 'appointment_request';
        }
        if (query.includes('garantia') || query.includes('warranty')) {
            return 'warranty_inquiry';
        }
        if (query.includes('reparar') || query.includes('arreglar') || query.includes('componer')) {
        ...
        if (query.includes('ayuda') || query.includes('help') || query.includes('auxilio')) {
            return 'help_request';
        }
    ...
    async createPersonalizedResponse(contextData) {
        ...
        let responseText = await this.populateTemplate(template, personalizationVars);
        
        // Analizar sentimiento y ajustar respuesta
        const sentimentAnalysis = this.analyzeSentiment(requestData.query);
        responseText = this.applySentimentAdjustments(responseText, sentimentAnalysis);
        
        // Aplicar ajustes de estilo según el cliente
        const styledResponse = this.applyPersonalityStyle(responseText, clientContext.personality);
        ...
        return {
            ...
            metadata: {
                responseType,
                personalized: personalizationVars.personalizationLevel > this.PERSONALIZATION_THRESHOLD,
                templateUsed: template.id,
                personalizationLevel: personalizationVars.personalizationLevel,
                contextualFactors: Object.keys(personalizationVars).filter(k => k !== 'personalizationLevel'),
    ...
    createPersonalizationVariables(contextInfo) {
        ...
        if (clientContext.name) {
            ...
            } else if (temporalContext.timeOfDay === 'afternoon' || temporalContext.timeOfDay === 'evening') {
        ...
        if (clientContext.customerType === 'returning' && conversationHistory.length > 0) {
        ...
        if (clientContext.history.totalInteractions > 3) personalizationLevel += 0.1;
    ...
    getResponseTemplate(responseType, clientContext) {
        const templates = this.responseTemplates.get(responseType) || this.responseTemplates.get('general_inquiry');
        
        // Seleccionar plantilla según estilo de comunicación del cliente
        const style = clientContext.personality.communicationStyle;
        const selectedTemplate = templates.find(t => t.style === style) || templates[0];
    ...
    async addContextualElements(baseResponse, contextData) {
        ...
        if (temporalContext.businessStatus === 'closed' &&
        ...
        if (clientContext.customerType === 'returning' &&
            !requestData.query.toLowerCase().includes('cita')) {
            enhancedResponse += ' ¿Te gustaría que te agende una cita para que no esperes?';
        }
        
        // Agregar información de garantía si es reparación
        if (requestData.query.toLowerCase().includes('reparar') &&
            !requestData.query.toLowerCase().includes('garantia')) {
            enhancedResponse += ' Todas nuestras reparaciones incluyen garantía de 30 días.';
        }
    ...
    async generateFallbackResponse(requestData) {
        ...
        return `${greeting}. Disculpa, estoy teniendo dificultades técnicas para procesar tu consulta. ` +
    ...
    async loadBusinessContext() {
        try {
            ...
            this.businessContext = {
                ...
                address: config.business.address || 'Av. C. del Refugio, fraccionamiento Valle de las Misiones',
                hours: config.business.hours || 'Lunes a Sábado de 11:00 AM a 9:00 PM, Domingos de 12:00 PM a 5:00 PM',
    ...
    enrichClientBehavior(context, requestData) {
        ...
        const urgencyIndicators = [
            /urgente/i, /rapido/i, /ya/i, /ahora/i, /inmediato/i,
            /emergency/i, /emergencia/i, /pronto/i
        ];
        ...
        const technicalTerms = [
            /digitalizador/i, /lcd/i, /oled/i, /amoled/i, /flex/i,
            /conector/i, /firmware/i, /software/i, /hardware/i
        ];
    ...
    analyzeSentiment(query) {
        const positiveWords = [
            /gracias/i, /excelente/i, /perfecto/i, /genial/i, /bueno/i,
            /contento/i, /feliz/i, /satisfecho/i
        ];
        
        const negativeWords = [
            /molesto/i, /enojado/i, /furioso/i, /malo/i, /terrible/i,
            /pesimo/i, /horrible/i, /decepcionado/i, /frustrado/i
        ];
    ...
    applySentimentAdjustments(responseText, sentimentAnalysis) {
        if (sentimentAnalysis.sentiment === 'negative' && sentimentAnalysis.confidence > 0.7) {
            ...
            responseText = `Lamento escuchar que estás teniendo problemas. Entiendo tu preocupación y estoy aquí para ayudarte. ` + responseText;
        } else if (sentimentAnalysis.sentiment === 'positive' && sentimentAnalysis.confidence > 0.7) {
            // Mantener el tono positivo
            if (!responseText.endsWith('!')) {
                 responseText = responseText.replace(/\.$/, '! Me alegra mucho poder ayudarte.');
            }
        }
        return responseText;
    }
    ...
    getContextStats() {
        ...
        for (const context of this.contextDatabase.values()) {
            ...
            context.history.deviceTypes.forEach(device => {
                stats.topDeviceTypes.set(device, (stats.topDeviceTypes.get(device) || 0) + 1);
            });
            
            // Contar tipos de servicios
            context.history.serviceTypes.forEach(service => {
                stats.topServiceTypes.set(service, (stats.topServiceTypes.get(service) || 0) + 1);
            });
            
            totalInteractions += context.history.totalInteractions;
        }
        
        stats.averageInteractions = stats.totalClients > 0 ?
            totalInteractions / stats.totalClients : 0;
    ...
    getMetrics() {
        return {
            ...this.metrics,
            personalizationRate: this.metrics.responsesGenerated > 0 ?
                this.metrics.personalizedResponses / this.metrics.responsesGenerated : 0,
            contextualRate: this.metrics.responsesGenerated > 0 ?
                this.metrics.contextualResponses / this.metrics.responsesGenerated : 0,
            memoryUsage: {
     ...
     */
    cleanupOldData(maxAge = 7 * 24 * 60 * 60 * 1000) { // 7 días por defecto
        ...
        try {
            if (exportData.contexts) {
                for (const [clientId, context] of Object.entries(exportData.contexts)) {
                    ...
                    context.history.deviceTypes = new Set(context.history.deviceTypes || []);
                    context.history.serviceTypes = new Set(context.history.serviceTypes || []);
            ...
            if (exportData.conversations) {
                for (const [clientId, conversation] of Object.entries(exportData.conversations)) {
            ...
            if (exportData.metrics) {
                ...
                this.metrics.templateUsage = new Map(Object.entries(exportData.metrics.templateUsage || {}));
            }
            
            logger.info('📥 Datos de contexto importados exitosamente', {
                contextsImported: Object.keys(exportData.contexts || {}).length,
                conversationsImported: Object.keys(exportData.conversations || {}).length
            });
    ...
    generateOptimizationSuggestions() {
        ...
        if(stats.topServiceTypes.size > 0){
            const topService = Array.from(stats.topServiceTypes.entries()).sort((a,b) => b[1] - a[1])[0][0];
            suggestions.push(`Considera crear una plantilla más específica para el servicio más popular: '${topService}'.`);
        }

        if(this.metrics.templateUsage.size > 10){
             suggestions.push(`Revisa las plantillas menos usadas para consolidar o mejorar su efectividad.`);
        }
