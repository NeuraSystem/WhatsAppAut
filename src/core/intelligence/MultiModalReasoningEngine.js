...
class MultiModalReasoningEngine {
    ...
    containsTechnicalTerms(query) {
        const technicalTerms = [
            /digitalizador/i, /lcd/i, /oled/i, /flex/i, /conector/i,
            /firmware/i, /ios/i, /android/i, /reset/i,
            /microsoldadura/i, /reballing/i, /ic/i, /chip/i,
            /mah/i, /voltios/i, /amperios/i, /ram/i, /gb/i
        ];
        ...
        try {
            ...
            switch(step.type) {
                ...
                case 'context_integration':
                    stepResult = await this.performContextIntegration(context, step.params);
                    break;
                case 'synthesis':
                    stepResult = await this.performSynthesis(context, currentResult, step.params);
                    break;
                default:
    ...
    async performTextAnalysis(context, params) {
        ...
        return {
            ...
            confidence: 0.85,
            evidence: [`Intent detected: ${intent}`, `Entities: ${JSON.stringify(entities)}`]
        };
    }
    ...
    async performImageAnalysis(context, params) {
        ...
        return {
            ...
            evidence: ["Análisis visual sugiere daño por agua. Requiere revisión humana."]
        };
    }
    ...
    async performKnowledgeLookup(context, params) {
        ...
        return {
            output: {
                deviceInfo: { model: 'iPhone 12', commonIssues: ['pantalla', 'bateria'] },
    ...
    async performPriceAnalysis(context, params) {
        ...
        return {
            output: {
                priceFound: true,
                priceInfo: { range: '$2500 - $3000 MXN', factors: ['calidad de la pieza'] }
            },
            confidence: 0.8,
            evidence: ["Rango de precios para cambio de pantalla de iPhone 12 encontrado."]
        };
    }
    ...
    async performSynthesis(context, currentResult, params) {
        ...
        return {
            ...
            evidence: ["Todos los pasos apuntan a una consulta de cotización estándar."]
        };
    }
    ...
    assessConfidence(analysisResult, queryClassification) {
        ...
        return {
            ...
            factors: ["Confianza basada en la síntesis de los pasos de razonamiento."]
        };
    }
    ...
    initializeReasoningStrategies() {
        this.reasoningStrategies.set('text_simple', {
            ...
            steps: [
                { name: 'text_analysis', type: 'text_analysis', outputKey: 'textAnalysis' },
                { name: 'knowledge_lookup', type: 'knowledge_lookup', outputKey: 'knowledge' },
        ...
        this.reasoningStrategies.set('text_complex', {
            ...
            steps: [
                { name: 'text_analysis', type: 'text_analysis', outputKey: 'textAnalysis' },
                { name: 'knowledge_lookup', type: 'knowledge_lookup', outputKey: 'knowledge' },
                { name: 'price_analysis', type: 'price_analysis', outputKey: 'pricing' },
                { name: 'context_integration', type: 'context_integration', outputKey: 'context' },
                { name: 'synthesis', type: 'synthesis', outputKey: 'final' }
            ]
        });
        ...
        this.reasoningStrategies.set('multimedia_simple', {
            ...
            steps: [
                { name: 'image_analysis', type: 'image_analysis', outputKey: 'imageAnalysis' },
                { name: 'text_analysis', type: 'text_analysis', outputKey: 'textAnalysis' },
                { name: 'synthesis', type: 'synthesis', outputKey: 'final' }
            ]
        });
