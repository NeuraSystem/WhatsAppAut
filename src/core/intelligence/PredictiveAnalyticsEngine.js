...
class PredictiveAnalyticsEngine {
    ...
    async generateProactiveActions(clientData) {
        ...
        if (customerType === 'vip' && history.totalInteractions > 10) {
            proactiveActions.immediateActions.push({ type: 'priority_support', description: 'Ofrecer canal de soporte prioritario', timing: 'inmediato', expectedImpact: 'alta satisfacción' });
        }
        
        if (history.urgencyLevel === 'high') {
            proactiveActions.immediateActions.push({ type: 'expedited_service', description: 'Ofrecer servicio express', timing: 'próxima consulta', expectedImpact: 'reducir estrés del cliente' });
        }
        
        if (history.deviceTypes?.size > 0) {
            proactiveActions.scheduledActions.push({ type: 'maintenance_reminder', description: 'Recordatorio de mantenimiento preventivo', timing: '3 meses', expectedImpact: 'prevenir problemas mayores' });
        }
        
        const seasonalFactors = this.getSeasonalFactors();
        if (seasonalFactors.currentSeason === 'winter') {
            proactiveActions.preventiveActions.push({ type: 'winter_care_tips', description: 'Consejos de cuidado de dispositivos en invierno', timing: 'próximas 2 semanas', expectedImpact: 'reducir daños por humedad/frío' });
        }
        
        if (customerType === 'returning') {
            proactiveActions.marketingOpportunities.push({ type: 'loyalty_program', description: 'Invitar a programa de lealtad', timing: 'próxima visita', expectedImpact: 'aumentar retención' });
        }
