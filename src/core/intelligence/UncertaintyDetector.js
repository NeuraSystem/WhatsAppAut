...
class UncertaintyDetector {
  constructor() {
    ...
    this.uncertaintyIndicators = {
      ...
      unknownModel: {
        weight: 0.8,
        description: "Modelo de dispositivo no reconocido",
      },
      complexQuery: { weight: 0.6, description: "Consulta compleja o ambigua" },
      priceNotFound: {
        weight: 0.7,
        description: "Precio no encontrado en base de datos",
      },
      customRequest: {
        weight: 0.8,
        description: "Solicitud personalizada o fuera de estándar",
      },
      technicalIssue: {
        weight: 0.6,
  ...
  async analyzeQuery(queryData) {
    ...
    try {
      ...
      analysis.needsEscalation =
        analysis.confidence < this.uncertaintyThreshold;

      if (analysis.needsEscalation) {
        analysis.escalationData = await this.prepareEscalationData(
          queryData,
          indicators,
        );
        ...
        analysis.agentCapabilities =
          await this.assessAgentCapabilities(queryData);
  ...
  async detectUncertaintyIndicators(queryData) {
    ...
    if (queryData.message?.hasMedia) {
      indicators.push({
        ...
        details: {
          mediaType: queryData.message.mediaType,
          reason: "Requiere análisis visual humano",
        },
      });
    }
    ...
    if (!modelAnalysis.found) {
      indicators.push({
        ...
        details: {
          extractedModel: modelAnalysis.extractedModel,
    ...
    // Verificar precios
    if (modelAnalysis.found) {
      const priceAnalysis = await this.analyzePriceAvailability(
        modelAnalysis.model,
        queryData.query,
      );
  ...
  analyzeQueryComplexity(query) {
    ...
    const complexityScore =
      Object.values(complexityFactors).filter(Boolean).length;
    return {
      isComplex: complexityScore >= 2,
      score: complexityScore,
      factors: Object.entries(complexityFactors)
  ...
  async analyzePriceAvailability(model, query) {
    ...
    return {
      ...
      reason: priceInfo
        ? "Precio encontrado"
        : "Precio no disponible en base de datos",
    };
  }
  ...
  detectCustomRequests(query) {
    ...
    return {
      ...
      requiresPersonalization:
        detectedIndicators.includes("negotiation") ||
        detectedIndicators.includes("discount"),
    };
  }
  ...
  analyzeTechnicalComplexity(query) {
    ...
    const containsProblemDescription = problemDescriptions.some((pattern) =>
      pattern.test(query),
    );
    const requiresExpertise =
      containsTechnicalTerms ||
      (containsProblemDescription && this.isComplexProblem(query));

    return {
      requiresExpertise,
      technicalTermsFound: containsTechnicalTerms,
      problemDescriptionFound: containsProblemDescription,
      reason: requiresExpertise
  ...
  async prepareEscalationData(queryData, indicators) {
    const aiAnalysis =
      indicators.length > 0
        ? indicators
            .map((i) => `${i.description} (${(i.weight * 100).toFixed(0)}%)`)
            .join(", ")
    ...
    return {
      ...
      media: queryData.message?.hasMedia
        ? { type: queryData.message.mediaType, present: true }
        : null,
      aiAnalysis,
      confidence: this.calculateConfidence(indicators),
      detectedIssues: indicators.map((i) => ({
        type: i.type,
  ...
  async assessAgentCapabilities(queryData) {
    ...
    if (modelAnalysis.found) {
      ...
      const priceInfo = await this.findPriceInDatabase(
        modelAnalysis.model,
        serviceType,
      );
    ...
    capabilities.canProvideServiceInfo = this.canProvideServiceInfo(
      queryData.query,
    );
    return capabilities;
  }
  ...
  async loadPriceDatabase() {
    try {
      ...
      logger.info("💰 Base de datos de precios cargada", {
        brands: this.priceDatabase.size,
      });
    } catch (error) {
    ...
    return {
      ...
      escalationRate:
        this.metrics.totalAnalyses > 0
          ? this.metrics.escalationsTriggered / this.metrics.totalAnalyses
          : 0,
