...
const AdminEscalationSystem = require('./intelligence/AdminEscalationSystem');
const AdaptiveLearningEngine = require('./intelligence/AdaptiveLearningEngine');
const UncertaintyDetector = require('./intelligence/UncertaintyDetector');
...
class OrchestrationController {
    ...
    async executeOperation(operation, context = {}) {
        ...
        try {
            ...
            if (systemStatus.degradationLevel > 2) {
                logger.warn(`OrchestrationController: Sistema en estado degradado (${systemStatus.degradationLevel}). Operación ${operationId} puede fallar.`);
            }

            // Apply performance optimizations if available
            let optimizedOperation = operation;
            if (this.performanceController) {
                optimizedOperation = await this.performanceController.optimizeOperation(operation, context);
            }

            // Execute operation with resilience protection
            let result;
            if (this.resilienceController) {
                result = await this.resilienceController.executeWithProtection(optimizedOperation, context);
            } else {
            ...
            logger.debug(`OrchestrationController: Operación ${operationId} completada exitosamente en ${responseTime}ms`);
            ...
            logger.error(`OrchestrationController: Operación ${operationId} falló después de ${responseTime}ms:`, error);
    ...
    handlePerformanceDegradation(performanceMetrics) {
        logger.warn('OrchestrationController: Degradación de performance detectada:', performanceMetrics);
        
        if (performanceMetrics.averageResponseTime > this.adaptiveThresholds.performanceDegradationTrigger) {
    ...
    handleSystemHealthChange(healthStatus) {
        logger.info('OrchestrationController: Cambio en salud del sistema:', healthStatus);
        ...
        } else if (this.performanceController && healthStatus.overall === 'HEALTHY') {
    ...
    updateSuccessMetrics(responseTime) {
        ...
        if (this.systemMetrics.averageResponseTime < config.performance?.responseTimeTarget) {
            this.systemMetrics.systemHealth = 'HEALTHY';
        }
    }
    ...
    updateFailureMetrics(error, responseTime) {
        ...
        const failureRate = this.systemMetrics.failedRequests / this.systemMetrics.totalRequests;
        ...
        } else if (failureRate > this.adaptiveThresholds.resilienceActivationThreshold) {
            this.systemMetrics.systemHealth = 'DEGRADED';
        }
    }
    ...
    async getSystemStatus() {
        ...
        const failureRate = status.metrics.totalRequests > 0
        ...
        if (failureRate > this.adaptiveThresholds.emergencyModeThreshold) {
            status.metrics.systemHealth = 'CRITICAL';
                   ...
                   status.metrics.averageResponseTime > this.adaptiveThresholds.performanceDegradationTrigger) {
    ...
    async optimizeSystem() {
        logger.info('OrchestrationController: Iniciando optimización del sistema...');
        ...
        try {
            ...
            if (this.performanceController) {
                ...
                logger.info('OrchestrationController: ✅ Optimización de performance completada');
            }

            // Optimize resilience layer
            if (this.resilienceController) {
                results.resilience = await this.resilienceController.optimize();
                logger.info('OrchestrationController: ✅ Optimización de resilience completada');
            }
            ...
            if (config.intelligence?.enableEscalation) {
                ...
                logger.info('OrchestrationController: ✅ Intelligence layer verificada');
            }

            logger.info('OrchestrationController: ✅ Optimización del sistema completada exitosamente');
            return results;

        } catch (error) {
            logger.error('OrchestrationController: ❌ Error durante optimización del sistema:', error);
            throw error;
        }
    }
    ...
    async shutdown() {
        try {
            logger.info('OrchestrationController: Iniciando apagado del sistema de orquestación...');
            ...
            this.isInitialized = false;
            logger.info('OrchestrationController: ✅ Sistema de orquestación apagado exitosamente');
        ...
        try {
            ...
            if (!config.intelligence?.enableEscalation) {
                ...
                return await this.executeOperation(
                    async (context) => context.fallbackResponse || 'Procesando consulta...',
            ...
            const autoDecision = await this.adaptiveLearningEngine.checkForAutoDecision(situation);
            ...
            const uncertaintyAnalysis = await this.uncertaintyDetector.analyzeQuery(queryData);
            
            if (uncertaintyAnalysis.needsEscalation) {
                // 3. Escalate to admin
                const escalationResponse = await this.adminEscalationSystem.escalateToAdmin(
            ...
            const fallbackResponse = await this.adminEscalationSystem.escalateToAdmin({
                customerInfo: queryData.customerInfo || { name: 'Cliente', phone: queryData.from },
    ...
    extractSituationFromQuery(queryData) {
        return {
            ...
            urgency: /urgente|rapido|ya|ahora/i.test(queryData.query) ? 'high' : 'normal'
        };
    }
    ...
    setupIntelligenceCoordination() {
        ...
        this.originalHandleAdminResponse = this.adminEscalationSystem.handleAdminResponse.bind(this.adminEscalationSystem);
        
        // Override to include learning
        this.adminEscalationSystem.handleAdminResponse = async (adminResponse, responseCallback) => {
    ...
    async recordAdminDecisionForLearning(adminResponse) {
        try {
            ...
            const currentEscalation = this.adminEscalationSystem.getCurrentEscalation();
            if (currentEscalation) {
                ...
                this.systemMetrics.learnedPatterns = this.adaptiveLearningEngine.getMetrics().totalPatterns;
            }
        } catch (error) {
    ...
    extractSituationFromEscalation(escalation) {
        return {
            ...
            timeOfDay: new Date().getHours(),
            urgency: escalation.originalQuery.toLowerCase().includes('urgente') ? 'high' : 'normal'
        };
    }
}
