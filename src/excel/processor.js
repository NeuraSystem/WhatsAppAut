...
function validateColumns(data) {
  ...
  for (const col of requiredColumns) {
    if (!columns.includes(col)) {
      throw new Error(
        `El archivo Excel no tiene la columna requerida: "${col}".`,
      );
    }
  }
}
...
async function insertDataIntoDB(data) {
  ...
  try {
    ...
    console.error(
      "Error durante la transacción, se revirtieron los cambios.",
      error,
    );
...
async function processExcelFile(filePath) {
  try {
    ...
    console.log(
      "Archivo Excel procesado e importado a la base de datos con éxito.",
    );
    return true;
  } catch (error) {
