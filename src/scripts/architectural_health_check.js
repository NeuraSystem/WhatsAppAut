...
async function architecturalHealthCheck() {
    ...
    try {
        ...
        if (missingVars.length === 0) {
            ...
            logger.info('✅ Configuración: Todas las variables de entorno están definidas');
        } else {
            results.configuration.environment = 'incomplete';
            logger.warn(`⚠️ Variables de entorno faltantes: ${missingVars.join(', ')}`);
        }

        // Overall health assessment
        const unhealthyServices = Object.values(results.services).filter(status => status !== 'healthy').length;
        const incompleteConfig = Object.values(results.configuration).filter(status => status !== 'complete').length;

        if (unhealthyServices === 0 && incompleteConfig === 0) {
            results.overall = 'healthy';
            logger.info('🎉 Estado general: SALUDABLE - Todos los servicios operativos');
        } else if (unhealthyServices <= 1 && incompleteConfig <= 1) {
            results.overall = 'degraded';
            logger.warn('⚠️ Estado general: DEGRADADO - Algunos servicios con problemas');
        } else {
            results.overall = 'critical';
            logger.error('🚨 Estado general: CRÍTICO - Múltiples servicios con problemas');
        }

        // Summary report
        logger.info('📊 Resumen del estado de salud:');
        logger.info(`   - Ollama: ${results.services.ollama || 'no verificado'}`);
        logger.info(`   - ChromaDB: ${results.services.chromadb || 'no verificado'}`);
        logger.info(`   - PostgreSQL: ${results.services.postgresql || 'no verificado'}`);
        logger.info(`   - Configuración: ${results.configuration.environment || 'no verificada'}`);
        logger.info(`   - Estado general: ${results.overall}`);

        return results;

    } catch (error) {
        logger.error('Error durante la verificación de salud arquitectural:', error);
