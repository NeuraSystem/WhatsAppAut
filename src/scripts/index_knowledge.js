...
const KNOWLEDGE_COLLECTION_NAME = process.env.KNOWLEDGE_COLLECTION_NAME || 'conocimiento_general';
const PERFIL_EMPRESARIAL_CHUNKS_PATH = path.join(__dirname, '../../data/processed_for_ai/PERFIL_EMPRESARIAL_SALVACELL_CHUNKS.json');
...
class LangChainEmbeddingAdapter {
    ...
    async generate(texts) {
        try {
            ...
            logger.error('Error en LangChainEmbeddingAdapter (index_knowledge):', error);
            throw error;
        }
    }
}
...
async function indexKnowledge() {
    ...
    try {
        ...
        if (!embeddingEngine) {
            logger.error('Motor de embeddings no inicializado. Asegúrate de que Ollama esté corriendo y el modelo esté disponible.');
            process.exit(1);
        }
        ...
        const conocimientosRes = await connection.query('SELECT id, texto FROM conocimientos');
        const conocimientos = conocimientosRes.rows;
        logger.info(`Obtenidos ${conocimientos.length} conocimientos generales de PostgreSQL.`);
        ...
        if (fs.existsSync(PERFIL_EMPRESARIAL_CHUNKS_PATH)) {
            logger.info(`Procesando chunks de perfil empresarial desde: ${PERFIL_EMPRESARIAL_CHUNKS_PATH}`);
            const perfilChunks = JSON.parse(fs.readFileSync(PERFIL_EMPRESARIAL_CHUNKS_PATH, 'utf-8'));
            ...
            perfilChunks.forEach((chunk) => {
                ...
                if (chunk.id && chunk.content && chunk.metadata) {
                    ...
                    logger.warn(`Chunk inválido encontrado en ${PERFIL_EMPRESARIAL_CHUNKS_PATH}:`, chunk);
                }
            });
            logger.info(`Añadidos ${perfilChunks.length} chunks de perfil empresarial.`);
        } else {
            logger.warn(`Archivo de chunks de perfil empresarial no encontrado: ${PERFIL_EMPRESARIAL_CHUNKS_PATH}`);
        }

        logger.info(`Preparados ${documentsToEmbed.length} documentos para indexación.`);
        ...
        logger.info(`Indexación completada. ${documentsToEmbed.length} documentos añadidos/actualizados en la colección '${KNOWLEDGE_COLLECTION_NAME}'.`);
