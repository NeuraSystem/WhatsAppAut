...
class MarkdownToPostgreSQLConverter {
    constructor() {
        this.markdownPath = path.join(__dirname, '../../data/processed_for_ai/precios_markdown');
    ...
    async verifyMarkdownFiles() {
        ...
        const files = fs.readdirSync(this.markdownPath).filter(file => file.endsWith('.md'));
    ...
    async processAllMarkdownFiles() {
        ...
        console.log(`📊 Procesamiento completado: ${this.migrationStats.successfulFiles}/${this.migrationStats.totalFiles} archivos`);
    }
    ...
    async processSingleMarkdownFile(fileName) {
        ...
        try {
            ...
            try {
                ...
                console.log(`   ✅ ${insertedCount} registros insertados, ${skippedCount} omitidos\n`);
    ...
    parseTableHeader(headerLine) {
        ...
        columns.forEach((col, index) => {
            ...
            } else if (lowerCol.includes('genérica') || lowerCol.includes('generica')) {
        ...
        if (originalPrice && originalPrice !== 'N/A') {
            ...
            if (precio > 0) {
                records.push({
                    ...
                    precio: precio,
                    tiempo_reparacion: 'Mismo día (antes 4PM) / Siguiente día (después 4PM)',
    ...
    async generateMigrationReport() {
        ...
        console.log(`📁 Archivos procesados: ${this.migrationStats.successfulFiles}/${this.migrationStats.totalFiles}`);
        console.log(`🗑️ Registros eliminados: ${this.cleanedCount}`);
        console.log(`📝 Registros insertados: ${this.migrationStats.totalRecords}`);
        console.log(`🔄 Duplicados omitidos: ${this.migrationStats.duplicatesSkipped}`);
        ...
        Object.entries(this.brandStats).forEach(([brand, stats]) => {
            console.log(`   ${brand}: ${stats.inserted} insertados (${stats.skipped} omitidos)`);
        });
        ...
        try {
            ...
            sampleResult.rows.forEach((row, index) => {
                console.log(`   ${index + 1}. ${row.modelo_celular} - $${row.precio} - ${row.notas.substring(0, 50)}...`);
            });
