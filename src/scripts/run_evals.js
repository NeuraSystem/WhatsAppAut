...
--- Ejecutando evaluación ${evalCase.id} (${evalCase.categoria}) ---`);
            ...
            try {
                ...
                agentResponse = typeof result === 'string' ? result : (result.response || JSON.stringify(result));
            ...
            if (isPassed) {
                ...
                logger.error(`   Esperaba contener: ${evalCase.respuesta_ideal_contiene.join(', ')}`);
        ...
        if (failedCount > 0) {
            logger.error('💥 ALGUNAS EVALUACIONES FALLARON. Revisa los logs para más detalles.');
            process.exit(1);
        } else {
