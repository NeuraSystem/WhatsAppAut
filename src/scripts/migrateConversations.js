...
class ConversationMigrator {
  ...
  async migrateAllConversations() {
    ...
    try {
      ...
      if (!initialized) {
        throw new Error(
          "No se pudo inicializar el sistema de memoria conversacional",
        );
      }
  ...
  extractConversationData(conversation) {
    ...
    const priceMatch = conversation.respuesta_enviada.match(
      /\$(\d+(?:,\d{3})*(?:\.\d{2})?)/,
    );
    if (priceMatch) {
    ...
    for (const { pattern, service } of servicePatterns) {
      if (
        pattern.test(conversation.consulta_original) ||
        pattern.test(conversation.respuesta_enviada)
      ) {
    ...
    const nameMatch = conversation.consulta_original.match(
      /(?:soy|me llamo|mi nombre es|con)\s+([A-Z][a-z]+)/i,
    );
    if (nameMatch) {
  ...
  async updateConversationWithChunkId(conversationId, chunkId) {
    try {
      ...
      logger.warn(
        `Error actualizando chunk_id para conversación ${conversationId}:`,
        error,
      );
    }
  }
  ...
  printMigrationSummary() {
    ...
    console.log(
      `🎯 Total procesadas: ${this.processedCount + this.errorCount + this.skippedCount}`,
    );
