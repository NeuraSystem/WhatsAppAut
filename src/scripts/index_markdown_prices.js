...
const PRICES_MARKDOWN_DIR = path.join(__dirname, '../../data/processed_for_ai/precios_markdown');
...
function processMarkdownFile(filePath) {
    ...
    for (let i = 0; i < sections.length; i++) {
        ...
        if (sectionTitle.includes('Información General')) {
            ...
            keywords.push('garantía', 'moneda', 'tiempo', 'domicilio', 'establecimiento');
...
async function indexMarkdownPrices() {
    try {
        ...
        const markdownFiles = fs.readdirSync(PRICES_MARKDOWN_DIR)
        ...
        try {
            ...
            logger.info(`📚 Usando colección existente: ${KNOWLEDGE_COLLECTION_NAME}`);
        } catch (error) {
            logger.info(`📚 Creando nueva colección: ${KNOWLEDGE_COLLECTION_NAME}`);
            collection = await chromaClient.createCollection({
                name: KNOWLEDGE_COLLECTION_NAME,
                metadata: { description: 'Conocimiento general incluyendo precios de reparaciones' }
            });
        }
        ...
        for (const filePath of markdownFiles) {
            try {
                ...
                logger.error(`   └─ ❌ Error procesando ${path.basename(filePath)}:`, error.message);
            }
        }
        ...
        logger.info(`   🔍 Prefijos de tarea habilitados: ${process.env.ENABLE_TASK_PREFIXES === 'true'}`);
...
async function cleanExistingPriceChunks() {
    try {
        ...
        if (results.ids && results.ids.length > 0) {
            ...
            logger.info(`   └─ Eliminados ${results.ids.length} chunks de precios existentes`);
        } else {
